/*
    ...
    You should have recebived a copy of the GNU Affero General Public License
...
/* Permissions (from the davical wiki):
    ...
    write-properties - grants access to update properties of the principal or collection. 
        In DAViCal, when granted to a user principal, 
        this will only grant access to update properties of the principal's collections and not the user principal itself. 
        When granted to a group or resource principal this will grant access to update the principal properties.
...
function netUnSubscribeCalendar (calendar_url) {
    /* 功能：取消订阅他人的日历
     * 输入：calendar_url:   他人日历的url；
     * 输出：无
     */
    
    // console.log("②webdav_func netUnSubscribeCalendar");
    var dataXML =   '<?xml version="1.0" encoding="UTF-8"?>' + 
                        '<C:share xmlns:C="http://calendarserver.org/ns/">' + 
                            '<C:remove>' +
                            '<A:href xmlns:A="DAV:">' + 
                                'urn:uuid:' + globalAccountSettings[0].cahref.slice(-37,-1) + 
                            '</A:href>' +
                            '</C:remove>' + 
                        '</C:share>';
    ...
    $.ajax({
        ...
        url:            calendar_url,
        cache:          false,
        crossDomain:    (typeof globalAccountSettings[0].crossDomain=='undefined' ? true : globalAccountSettings[0].crossDomain),
        timeout:        globalAccountSettings[0].timeOut,
        username:       (globalSettings.usejqueryauth.value==true ? globalAccountSettings[0].userAuth.userName     : null),
        password:       (globalSettings.usejqueryauth.value==true ? globalAccountSettings[0].userAuth.userPassword : null),
        ...
        beforeSend: function (req) {
            if (globalSettings.usejqueryauth.value              != true && 
                globalAccountSettings[0].userAuth.userName      != ''   && 
                globalAccountSettings[0].userAuth.userPassword  != '') {

                req.setRequestHeader('Authorization', basicAuth (globalAccountSettings[0].userAuth.userName, globalAccountSettings[0].userAuth.userPassword));
            }
            req.setRequestHeader('X-client', globalXClientHeader);
            req.setRequestHeader('Depth', '0');
        },
        error: function (objAJAXRequest, strError) {
            console.log("Error: [netUnSubscribeCalendar: 'post " + globalAccountSettings[0].cahref+"'] code: '" + 
...
function netSubscribeCalendar (calendar_url, commonname, summary) {
    // 功能：订阅他人的日历
    // 输入：calendar_url:   他人日历的url；
    //      commonname：    ?自己对于他人日历的?；
    //      summary：       自己对于他人日历的重命名；
    // 输出：无

        // console.log("②webdav_func netSubscribeCalendar");
        var dataXML =   '<?xml version="1.0" encoding="UTF-8"?>' + 
                            '<C:share xmlns:C="http://calendarserver.org/ns/">' + 
                                '<C:set>' +
                                    '<A:href xmlns:A="DAV:">urn:uuid:' + globalAccountSettings[0].cahref.slice(-37,-1) + '</A:href>' +
                                    '<C:common-name>' + commonname + '</C:common-name>' +
        ...
        $.ajax({
            ...
            error: function (objAJAXRequest, strError) {
                console.log("Error: [netSubscribeCalendar: 'post " + globalAccountSettings[0].cahref+"'] code: '" + 
                    objAJAXRequest.status+"' status: '"+strError+"'" + 
                    (objAJAXRequest.status==0 ? ' (this error code usually means network connection error,' + 
                        ' or your browser is trying to make a cross domain query,' + 
                        ' but it is not allowed by the destination server or the browser itself)' : ''));
...
function netUnShareCalendar (inputCollection) {
    // 功能：调用 acl 方法，将共享日历设置为私有日历（他人可订阅）
    // 输入：inputCollection:日历资源
    // 输出：无
     
    // console.log("②webdav_func netUnShareCalendar");
    var dataXML =   '<?xml version="1.0" encoding="utf-8"?>' + 
                        '<D:acl xmlns:D="DAV:"/>';
    ...
    $.ajax({
        ...
        xhrFields:      { withCredentials: (typeof inputCollection.withCredentials=='undefined' ? false : inputCollection.withCredentials) },
        ...
        beforeSend: function (req) {
            if (globalSettings.usejqueryauth.value      != true && 
                inputCollection.userAuth.userName       != ''   && 
                inputCollection.userAuth.userPassword   != '') {
        ...
        error: function (objAJAXRequest, strError) {
            console.log("Error: [netUnShareCalendar: 'acl " + inputCollection.url + inputCollection.href + "'] code: '" + 
                objAJAXRequest.status + "' status: '" + strError + "'" + 
                (objAJAXRequest.status==0 ? ' (this error code usually means network connection error,' + 
                    ' or your browser is trying to make a cross domain query,' + 
                    ' but it is not allowed by the destination server or the browser itself)' : ''));
...
function netShareCalendar (inputCollection_url) {
    // 功能：调用 acl 方法，将日历设置为共享日历（他人可订阅）
    // 输入：inputCollection_url: 日历资源的url
    // 输出：无
     
    // console.log("②webdav_func netShareCalendar");
    var dataXML =   '<?xml version="1.0" encoding="utf-8"?>' + 
                        '<D:acl xmlns:D="DAV:">' +
                            '<D:ace>' +
                                '<D:principal>' +
    ...
    $.ajax({
        ...
        error: function (objAJAXRequest, strError) {
            console.log("Error: [netShareCalendar: 'acl " + inputCollection.url + inputCollection.href + "'] code: '" + 
                objAJAXRequest.status + "' status: '" + strError + "'" + 
                (objAJAXRequest.status==0 ? ' (this error code usually means network connection error,' + 
                    ' or your browser is trying to make a cross domain query,' + 
                    ' but it is not allowed by the destination server or the browser itself)' : ''));
...
function netSearchCalendar (match) {
    // 功能：调用 report 方法，搜索可订阅的他人日历
    // 输入：match:用于匹配的字符串（暂时只有e-mail可用）
    // 输出：无
    
    // console.log("②webdav_func netSearchCalendar");
    var dataXML =   '<?xml version="1.0" encoding="utf-8" ?>' + 
                    '<D:principal-property-search xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav" xmlns:CS="http://calendarserver.org/ns/">'+
                        '<D:property-search>' +
                            '<D:prop>' +
    ...
    function ajaxComplete_Search (data, textStatus, xml) {
        ...
        var dataXML =   '<?xml version="1.0" encoding="utf-8"?>' + 
                    '<D:propfind xmlns:D="DAV:">' + 
                        '<D:prop>' + 
                            ...
                            '<D:current-user-privilege-set class="java.util.LinkedList"/>' +
                            '<D:displayname/>' +
                            '<D:resourcetype/>' +
        ...
        var searched_href = $(xml.responseXML).children().filterNsNode('multistatus').children()
            .filterNsNode('response').children().filterNsNode('propstat').children()
            .filterNsNode('prop').children().filterNsNode('calendar-home-set').children()
            .filterNsNode('href').html();

        var searched_displayname = $(xml.responseXML).children().filterNsNode('multistatus').children()
            .filterNsNode('response').children().filterNsNode('propstat').children()
            .filterNsNode('prop').children().filterNsNode('displayname').html();

        var searched_CUT = $(xml.responseXML).children().filterNsNode('multistatus').children()
            .filterNsNode('response').children().filterNsNode('propstat').children()
            .filterNsNode('prop').children().filterNsNode('calendar-user-type').html();
        ...
        $.ajax({
            ...
            error: function (objAJAXRequest, strError) {
                console.log("Error: [netSearchCalendar: 'PROPFIND "+uidFull+"']: code: '"+objAJAXRequest.status+"' status: '"+strError + 
                    "'"+(objAJAXRequest.status==0 ? ' - see https://www.inf-it.com/'+globalAppName.toLowerCase()+'/readme.txt (cross-domain setup)' : ''));
            },
            success: function (data, textStatus, xml) {
                // 解析 propfind 返回的用户可被订阅的日历相关信息：包括日历的 href 和 displayname 和 calendar-color
                // var calendar_href = new Array();
                // var calendar_displayname = new Array();
                // var calendar_color = new Array();
                var re       = new RegExp('^(https?://)([^/]+)(.*)','i');
                var tmp      = globalAccountSettings[0].cahref.match(re);
                var baseHref = tmp[1] + tmp[2];
                var tmp_html = "";

                $(xml.responseXML).children().filterNsNode('multistatus').children().each(function(index, el) {
                    if ($(el).find('resourcetype').children().filterNsNode('calendar').length==1 && 
                        $(el).find('resourcetype').children().filterNsNode('collection').length==1 && 
                        $(el).find('supported-calendar-component-set').children()[0].getAttribute('name') === 'VEVENT') {
                        // 'resourcetype' 字段下只有一个 calendar 和 collection ，并且 name 字段为 VEVENT 的才是日历
                        
                        tmp_html += "<li data-id='" + baseHref + $(el).children('href').text() + "''>" + 
                                        "<strong>" + 
                        ...
                        // calendar_href.push($(el).children('href').text());
                        // calendar_displayname.push($(el).children('propstat').children('prop').children('displayname').text());
                        // if ($(el).children('propstat').children('prop').children('calendar-color').text()) {
                        //     // ╮(╯▽╰)╭，不确定有没有
                        //     calendar_color.push($(el).children('propstat').children('prop').children('calendar-color').text());
                        // }
                        // else {
                        //     calendar_color.push('#000000');
                        // }
                    }
                });

                $('#addOthersCalendarDetail .others-calendars-ul').html(tmp_html);
                $('#addOthersCalendarDetail .others-calendars-ul li').click(function() {
                    // 日历详情中的各项日历的点击事件
    ...
    $.ajax({
        type:           'report',
        url:            globalAccountSettings[0].cahref,
        cache:          false,
        crossDomain:    (typeof globalAccountSettings[0].crossDomain=='undefined' ? true : globalAccountSettings[0].crossDomain),
        timeout:        globalAccountSettings[0].timeOut,
        username:       (globalSettings.usejqueryauth.value==true ? globalAccountSettings[0].userAuth.userName     : null),
        ...
        success: ajaxComplete_Search
    });
}

function netDeleteCalendar (inputCollection) {
    // 功能：调用 delete 方法删除日历
    // 输入：inputCollection：日历资源
    // 输出：无
    
    // console.log("②webdav_func netDeleteCalendar");
    var dataXML = '';
    ...
    $.ajax({
        ...
        success: function (data, textStatus, xml) {
            // 将删除后的日历显示出来,调用 reloadResources 函数更新即可
            reloadResources();
        }
    });
}

function netMakeCalendar (displayName, isShared, calendarColor) {
    // 功能：调用 mkcalendar 方法新建日历
    // 输入：displayName:   日历名称, 
    //      isShared:      是否共享
    //      calendarColor: 日历颜色
    // 输出：无
    
    // console.log("②webdav_func netMakeCalendar");
    var dataXML =   '<?xml version="1.0" encoding="UTF-8"?>' +
                        ...
                        '<A:set xmlns:A="DAV:">' +
                            '<A:prop>' + 
                                '<A:displayname>' + 
                                    displayName + 
                                '</A:displayname>' + 
                                '<G:calendar-color xmlns:G="http://apple.com/ns/ical/">' + 
                                    calendarColor + 
                                '</G:calendar-color>' + 
                                '<B:supported-calendar-component-set>' + 
                                    '<B:comp name="VEVENT"/>' +
                                '</B:supported-calendar-component-set>' +
                                '<G:calendar-order xmlns:G="http://apple.com/ns/ical/">1</G:calendar-order>' +
    ...
    var newcalendar_url = globalAccountSettings[0].cahref + Math.uuid() + '/';
    ...
    $.ajax({
        ...
        error: function (objAJAXRequest, strError) {
            console.log("Error: [netMakeCalendar: 'MKCALENDAR " + globalAccountSettings[0].cahref+"'] code: '" + 
                objAJAXRequest.status+"' status: '"+strError+"'" + 
                (objAJAXRequest.status==0 ? ' (this error code usually means network connection error,' + 
                    ' or your browser is trying to make a cross domain query,' + 
                    ' but it is not allowed by the destination server or the browser itself)' : ''));
        ...
        success: function (data, textStatus, xml) {
            // 将新增日历显示出来,调用 reloadResources 函数更新即可
            if (isShared) {
                netShareCalendar(newcalendar_url);
            } 
            else {
                reloadResources();
            }
...
function netSaveProperty (inputCollection, hrefProperty, inputProperty, inputValue) {
    // 功能：Save the collection property (stored as DAV property on server)
    //      调用 PROPPATCH 方法， 保存日历属性（名称和颜色）
    // 输入：inputCollection：日历对象, 
    //      hrefProperty：命名空间（名称使用 ‘DAV:’、颜色使用 'http://apple.com/ns/ical/'） 
    //      inputProperty：要修改的属性名称（日历名称 displayname 、日历颜色 calendar-color）, 
    //      inputValue：要修改的属性值（日历名称 displayname 、日历颜色 calendar-color）
    // 输出：
    
    // console.log("②webdav_func netSaveProperty");
    var dataXML =   '<?xml version="1.0" encoding="utf-8"?>' + 
                    '<D:propertyupdate xmlns:D="DAV:">' + 
                    '<D:set>' + 
                    '<D:prop>' + 
                        '<I:' + inputProperty + ' xmlns:I="' + hrefProperty + '">' + 
                            inputValue + 
                        '</I:' + inputProperty + '>' +
                    '</D:prop>' + 
                    '</D:set>' + 
    ...
    $.ajax({
        ...
        error: function (objAJAXRequest, strError) {
            console.log("Error: [netSaveProperty: 'PROPPATCH " + inputCollection.url+inputCollection.href+"'] code: '" + 
                objAJAXRequest.status+"' status: '"+strError+"'" + 
                (objAJAXRequest.status==0 ? ' (this error code usually means network connection error,' + 
                    ' or your browser is trying to make a cross domain query,' + 
                    ' but it is not allowed by the destination server or the browser itself)' : ''));
        ...
        success: function (data, textStatus, xml) {
            // 1.将新标题应用于日历：2.1.修改 fcSource 中的 events 的 displayValue；
            //                     2.2.修改左侧日历列表中的日历标题；
            if (inputProperty === "displayname") {
                var title = inputValue;
                inputCollection.displayvalue = title;
                // 2.1.修改 fcSource 中的 events 的 displayValue
                if (inputCollection.fcSource != null) {
                    for (var arr in inputCollection.fcSource.events) {
                        arr.displayvalue = title;
                    }
                }
                // 2.2.修改左侧日历列表中的日历标题
                var tmp_dom_node = document.getElementById(inputCollection.uid).parentElement;
                $(tmp_dom_node).find('.ListItemTitle').html(title);
            ...
            // 2.将新颜色应用于日历：2.1.修改 fcSource 中的各种颜色；
            //                     2.2.修改左侧日历列表中的日历颜色；
            //                     2.3.重新获取事件
            if (inputProperty === "calendar-color") {
                var color = inputValue;
                // 2.1.修改 fcSource 中的各种颜色
                ...
                // 2.2.刷新左侧日历列表
                var tmp_dom_node = document.getElementById(inputCollection.uid).parentElement;
                $(tmp_dom_node).find('.regular-checkbox + label').css('border-color', color);
                $(tmp_dom_node).find('.labelafter').css('background', color);
...
function netCheckAndCreateConfiguration (configurationURL) {
    // 功能：create config from globalNetworkCheckSettings
    //      根据 globalNetworkCheckSettings 的内容，自己创建XML文件通过 PROPFIND 命令发送至服务器，读取并创建配置
    // 输入：configurationURL
    // 输出：？
    // console.log("②webdav_func netCheckAndCreateConfiguration");
    ...
    $.ajax({
        ...
        xhrFields:      { withCredentials: (typeof configurationURL.withCredentials=='undefined' ? false : configurationURL.withCredentials) },
        ...
        error: function (objAJAXRequest, strError) {
            console.log("Error: [netCheckAndCreateConfiguration: 'PROPFIND "+configurationURL.href+"'] code: '" + 
                objAJAXRequest.status+"' status: '"+strError+"'" + 
                (objAJAXRequest.status==0 ? 
                    ' - see https://www.inf-it.com/'+globalAppName.toLowerCase()+'/readme.txt (cross-domain setup)' : 
                    ''));

            $('.ero_tixing').css('display', 'block');
            $('#LoginLoader').fadeOut(1200);
            return false;
        },
        success: function (data, textStatus, xml) {
            $('.ero_tixing').css('display', 'none');
            var count = 0;
            if ($(xml.responseXML).children().filterNsNode('multistatus').children()
                .filterNsNode('response').children().filterNsNode('propstat').children()
                .filterNsNode('status').text().match(RegExp('200 OK$'))) {
                ...
                if (typeof(globalAccountSettingsHook) == 'function') {
                    // Hook for globalAccountSettings (openCRX)
                ...
                if (configurationURL.additionalResources != undefined && 
                    configurationURL.additionalResources.length > 0) {
...
function netLoadConfiguration (configurationURL) {
    // 功能：Load the configuration from XML file
    //      从XML中读取配置
    // 输入：configurationURL
    // 输出：
    
    // console.log("②webdav_func netLoadConfiguration");
    $.ajax({
        type: 'GET',
        ...
        dataType: 'xml',
        error: function (objAJAXRequest, strError) {
            console.log("Error: [loadConfiguration: 'GET "+configurationURL.href+"'] code: '"+objAJAXRequest.status + 
                "' status: '"+strError+"'" + 
                (objAJAXRequest.status==0 ? 
                    ' - see https://www.inf-it.com/'+globalAppName.toLowerCase()+'/readme.txt (cross-domain setup)' : 
                    ''));
            ...
            $(xml.responseXML).children('resources').children('resource').each(
                function (index, element) {
                    if ($(element).children().filterNsNode('type').children().filterNsNode('addressbook').length == 1 || 
                        $(element).children().filterNsNode('type').children().filterNsNode('calendar').length == 1) {
                        ...
                        if (tmp.text() == 'true') {
                            ...
                            tmp.children('collection').each(
                                function (index, element) {
                                    ...
                                    if ((matched=$(element).text().match(rex))!=null && 
                                        matched.length==3) {
                        ...
                        globalAccountSettings[globalAccountSettings.length] =   {   type:                   'network', 
                                                                                    href:                   href, 
                                                                                    hrefLabel:              hreflabel, 
                                                                                    ...
                                                                                    extendedDelegation:     extendedDelegation, 
                                                                                    ignoreAlarms:           ignoreAlarms, 
                                                                                    backgroundCalendars:    backgroundCalendars, 
                                                                                    collectionTypes:        collectionTypes, 
                                                                                    ignoreBound:            ignorebound };
                        count++;
                    }
                }
            );
...
function DAVresourceDelegation (inputResource, index, lastIndex) {
    ...
    // console.log("②webdav_func DAVresourceDelegation");
    ...
    var uidFull         = tmp[1] + inputResource.userAuth.userName + '@' + tmp[2] + tmp[3]; 
        //for the error handler
    var settingsXML     = '';
    var delegationXML   = '';

    if (typeof inputResource.extendedDelegation!='undefined' && inputResource.extendedDelegation) {
        if (inputResource.href.indexOf(globalLoginUsername)!=-1 && inputResource.settingsAccount && 
            (globalSettings.settingstype.value=='' || globalSettings.settingstype.value==null || 
                (globalSettings.settingstype.value!='' && globalSettings.settingstype.value!=null && 
                    globalSettings.settingstype.value=='principal-URL'))) {

            settingsXML = '<D:property name="settings" namespace="http://inf-it.com/ns/dav/"/>';
        }
        delegationXML = '<?xml version="1.0" encoding="utf-8"?>' + 
                        '<D:expand-property xmlns:D="DAV:">' +
                            '<D:property name="calendar-proxy-read-for" namespace="http://calendarserver.org/ns/">' +
                            '<D:property name="resourcetype"/>' + 
                            '<D:property name="current-user-privilege-set"/>' + 
                            '<D:property name="displayname"/>' + 
                            '<D:property name="calendar-user-address-set" namespace="urn:ietf:params:xml:ns:caldav"/>' + 
                        ...
                        '</D:property>' + 
                            '<D:property name="calendar-proxy-write-for" namespace="http://calendarserver.org/ns/"><D:property name="resourcetype"/>' + 
                            '<D:property name="current-user-privilege-set"/>' + 
                            '<D:property name="displayname"/>' + 
                            '<D:property name="calendar-user-address-set" namespace="urn:ietf:params:xml:ns:caldav"/>' + 
                            '<D:property name="calendar-home-set" namespace="urn:ietf:params:xml:ns:caldav"/>' + 
                            '<D:property name="addressbook-home-set" namespace="urn:ietf:params:xml:ns:carddav"/>' + 
                        '</D:property>' + settingsXML + 
                            '<D:property name="resourcetype"/>' + 
                            '<D:property name="current-user-privilege-set"/>' + 
                            '<D:property name="displayname"/>' + 
                            '<D:property name="calendar-user-address-set" namespace="urn:ietf:params:xml:ns:caldav"/>' + 
    ...
    function ajaxComplete_Delegation (data, textStatus, xml) {
        // 功能：（DAVresourceDelegation 函数内）
        // 输入：data, 
        //      textStatus, 
        //      xml
        // 输出：

        // console.log("②webdav_func ajaxComplete_Delegation");
        if (typeof globalAccountSettings == 'undefined') {
            // 创建 globalAccountSettings
            globalAccountSettings = [];
        }
        ...
        for (var m=0; m<searchR.length; m++) {
            ...
            function (dindex,delement) {
                ...
                if (!found) {
                    ...
                    globalAccountSettings[globalAccountSettings.length-1].userAuth  = {userName: inputResource.userAuth.userName, 
                                                                                       userPassword: inputResource.userAuth.userPassword};
                }
                if (typeof inputResource.extendedDelegation!='undefined' && inputResource.extendedDelegation) {
                    ...
                    var addressbook_home = $(delement).children().filterNsNode('propstat').children()
                        .filterNsNode('prop').children().filterNsNode('addressbook-home-set').children().filterNsNode('href').text();

                    if (addressbook_home == '') {
                        // addressbook-home-set has no 'href' value -> SabreDav
                        addressbook_home = $(delement).children().filterNsNode('href')
                                                .text().replace('/principals/users/caldav.php','/caldav.php');
                    }

                    if (addressbook_home.match(RegExp('^https?://','i')) != null) {
                    ...
                    if (calendar_home.match(RegExp('^https?://','i')) != null) {
                        // absolute URL returned
        ...
        if (typeof inputResource.extendedDelegation!='undefined' && inputResource.extendedDelegation && !settingsLoaded && 
            inputResource.href.indexOf(globalLoginUsername)!=-1 && inputResource.settingsAccount && 
            (globalSettings.settingstype.value=='' || globalSettings.settingstype.value==null || 
                (globalSettings.settingstype.value!='' && globalSettings.settingstype.value!=null && globalSettings.settingstype.value=='principal-URL'))) {

            var settings = $(xml.responseXML).children().filterNsNode('multistatus').children().filterNsNode('response').children()
                .filterNsNode('propstat').children().filterNsNode('prop').children().filterNsNode('settings').text();
        ...
        if (typeof inputResource.extendedDelegation!='undefined' && inputResource.extendedDelegation) {
            ...
            $(xml.responseXML).children().filterNsNode('multistatus').children().filterNsNode('response').children()
                .filterNsNode('propstat').children().filterNsNode('prop').children().filterNsNode('calendar-user-address-set').children().each(
            ...
            var addressbook_home = response.children().filterNsNode('propstat').children().filterNsNode('prop').children()
                .filterNsNode('addressbook-home-set').children().filterNsNode('href').text();
            ...
            var calendar_home = response.children().filterNsNode('propstat').children().filterNsNode('prop').children()
                .filterNsNode('calendar-home-set').children().filterNsNode('href').text();
    ...
    // 第一次尝试获取缓存数据（如果在“auth module”的回复中可获取缓存结果）
    var tmpCache;
    var tmpDav = inputResource.href.match('^(.*/)([^/]+)/$');

    if (globalXMLCache!=null && 
        (tmpCache=globalXMLCache.children('davprincipaldelegation[request_url="' + 
            jqueryEscapeSelector(tmpDav[1]+encodeURIComponent(tmpDav[2])+'/')+'"]').remove()).length) {
        ...
        ajaxComplete_Delegation ('', 'success', {responseXML: tmpCache});
    }
    else {
        if (typeof globalDebug!='undefined' && globalDebug instanceof Array && globalDebug.indexOf('cache') != -1) {
        ...
        $.ajax({
            ...
            xhrFields:      { withCredentials: (typeof inputResource.withCredentials=='undefined' ? false: inputResource.withCredentials) },
            ...
            error: function (objAJAXRequest, strError) {
                console.log("Error: [DAVresourceDelegation: 'REPORT " + uidFull+"'] code: '"+objAJAXRequest.status+"' status: '"+strError+"'" + 
                    (objAJAXRequest.status==0 ? ' (this error code usually means network connection error, or your browser is trying to make a cross domain query, but it is not allowed by the destination server or the browser itself)': ''));
            },
            success:        ajaxComplete_Delegation
        });
    }
}
...
function netFindResource (inputResource, inputResourceIndex, forceLoad, indexR, loadArray) {
    ...
    // console.log("②webdav_func netFindResource");
    ...
    if (indexR < globalAccountSettings.length) {
        ...
        if ((isAvaible('CardDavMATE') && globalCardDAVInitLoad) || 
            (isAvaible('CalDavZAP') && globalCalDAVInitLoad) || 
            (isAvaible('Projects') && !isProjectsLoaded) || 
            (isAvaible('Settings') && !isSettingsLoaded)) {

            $('#MainLoaderInner').html(localization[globalInterfaceLanguage].loadingResources
                .replace('%act%', globalResourceNumberCount).replace('%total%', globalResourceNumber));
        }
    }

    if ((   typeof inputResource!='undefined' && 
            typeof inputResource.collectionTypes!='undefined' && 
            inputResource.collectionTypes!=null && 
            (   inputResource.collectionTypes.indexOf('calendar')==-1) && 
                inputResource.collectionTypes.indexOf('addressbook')==-1) || 
        (   typeof inputResource!='undefined' && 
            typeof loadArray!='undefined' && 
            loadArray!=null && loadArray.indexOf(inputResource.href)==-1)) {
            ...
            if (isAvaible('CalDavZAP')) {
                if (!globalDefaultCalendarCollectionActiveAll) {
                    for (var i=0; i<globalSettings.activecalendarcollections.value.length; i++) {
                        if (typeof globalCrossServerSettingsURL!='undefined' && 
                            globalCrossServerSettingsURL!=null && 
                ...
                if (globalDefaultCalendarCollectionActiveAll) {
                    for (var i=0; i<globalResourceCalDAVList.collections.length; i++) {
                        if (globalResourceCalDAVList.collections[i].uid!=undefined && 
                            $('#ResourceCalDAVList input[data-id="'+globalResourceCalDAVList.collections[i].uid+'"]:visible').length>0) {
                ...
                if (!globalDefaultTodoCalendarCollectionActiveAll) {
                    ...
                    if (globalSettings.activetodocollections.value.length>0 && 
                        globalVisibleCalDAVTODOCollections.length==0) {
                ...
                if (globalDefaultTodoCalendarCollectionActiveAll) {
                    for (var i=0; i<globalResourceCalDAVList.TodoCollections.length; i++) {
                        if (globalResourceCalDAVList.TodoCollections[i].uid!=undefined && 
                            $('#ResourceCalDAVTODOList input[data-id="'+globalResourceCalDAVList.TodoCollections[i].uid+'"]:visible').length>0) {
                ...
                if ($('#ResourceCalDAVList .resourceCalDAV_item[data-id]:visible').length==0 && 
                    globalResourceCalDAVList.collections.length>0) {
                    ...
                    for (var c=0; c<globalResourceCalDAVList.collections.length; c++) {
                        if (globalResourceCalDAVList.collections[c].uid!=undefined) {
                            ...
                            if (globalAccountSettings[0].href==resourceCalDAV_href && 
                                globalAccountSettings[0].userAuth.userName==globalResourceCalDAVList.collections[c].userAuth.userName) {
                ...
                if ($('#ResourceCalDAVTODOList .resourceCalDAVTODO_item[data-id]:visible').length==0 && 
                    globalResourceCalDAVList.TodoCollections.length>1) {
                    ...
                    for (var c=0; c<globalResourceCalDAVList.TodoCollections.length; c++) {
                        if (globalResourceCalDAVList.TodoCollections[c].uid!=undefined) {
                            ...
                            var resourceCalDAV_href = tmp[1]+tmp[3]+tmp[4];
                            if (globalAccountSettings[0].href==resourceCalDAV_href && 
                                globalAccountSettings[0].userAuth.userName==globalResourceCalDAVList.TodoCollections[c].userAuth.userName)
                    ...
                    for (var c=0; c<enabledArray.length; c++) {
                        ...
                        var resDOMItem = $('#ResourceCalDAVTODOList').find('.resourceCalDAVTODO_item[data-id="' + 
                            jqueryEscapeSelector(enabledArray[c].uid) + '"]');
        ...
        if (isAvaible('CalDavZAP')) {
            ...
            for (var i=0; i<cals.length; i++) {
                if (cals[i].uid != undefined) {
                    calendarsArray[calendarsArray.length] = {displayValue:cals[i].displayvalue,uid:cals[i].uid, 
                                                             permissions_read_only:cals[i].permissions.read_only, 
                                                             makeLoaded:cals[i].makeLoaded};
                }
            }
    ...
    if (inputResource.href.indexOf(globalLoginUsername)!=-1 && 
        inputResource.settingsAccount && 
        (globalSettings.settingstype.value=='' || 
            globalSettings.settingstype.value==null || 
            (globalSettings.settingstype.value!='' && 
                globalSettings.settingstype.value!=null && 
                globalSettings.settingstype.value=='principal-URL'))) {
    ...
    var baseHref = tmp[1]+tmp[2];
    if (typeof inputResource.extendedDelegation!='undefined' && 
        inputResource.extendedDelegation && 
        (typeof inputResource.abhref!='undefined' || typeof inputResource.cahref!='undefined')) {
    ...
    $.ajax({
            ...
            if (globalSettingsSaving != '' || 
                ...
                if (globalSettingsSaving!='' || 
                    (isAvaible('Settings') && 
                        ...
                        $('.resourceSettings_item_selected').attr('data-type')=='setting_group_password')) {

                    indexR++;
                    if (((isAvaible('CardDavMATE') && globalCardDAVInitLoad) || 
                            (isAvaible('CalDavZAP') && globalCalDAVInitLoad)) && 
                        indexR==globalAccountSettings.length) {
        ...
        data:   '<?xml version="1.0" encoding="utf-8"?>' + 
                '<D:propfind xmlns:D="DAV:">' + 
                    '<D:prop>' + settingsXML + 
                        ...
                        '<D:displayname/>' + 
                        '<D:resourcetype/>' + 
                        '<L:calendar-home-set xmlns:L="urn:ietf:params:xml:ns:caldav"/>' + 
                        '<R:addressbook-home-set xmlns:R="urn:ietf:params:xml:ns:carddav"/>' + 
                    '</D:prop>' + 
                '</D:propfind>',

        dataType: 'xml',
        error: function (objAJAXRequest, strError) {
            console.log("Error: [netFindResource: 'PROPFIND " + uidFull + "']: code: '" + objAJAXRequest.status + "' status: '" + strError + "'" + 
                (objAJAXRequest.status==0 ? ' - see https://www.inf-it.com/'+globalAppName.toLowerCase()+'/readme.txt (cross-domain setup)' : ''));
            ...
            for (var i=0; i< globalAccountSettings.length; i++) {
                if (typeof globalAccountSettings[i].errorLoaded=='undefined' || 
                    ...
                    globalAccountSettings[i].errorLoaded===false) {
        ...
        success: function (data, textStatus, xml) {
            ...
            if (!settingsLoaded && 
                inputResource.href.indexOf(globalLoginUsername)!=-1 && 
                inputResource.settingsAccount && 
                (   globalSettings.settingstype.value=='' || globalSettings.settingstype.value==null || 
                    (   globalSettings.settingstype.value != ''   && 
                        globalSettings.settingstype.value != null && 
            ...
            else if (!globalSyncSettingsSave && inputResource.href.indexOf(globalLoginUsername)!=-1 && 
                    ((isAvaible('CardDavMATE') && globalCardDAVResourceSync) || 
                        (isAvaible('CalDavZAP')&&globalCalDAVResourceSync))) {

                globalSyncSettingsSave = true;
                var loadedCals      = new Array(), 
                    loadedTodoCals  = new Array(), 
                    loadedAddrs     = new Array();
            ...
            var addressbook_home = response.children().filterNsNode('propstat').children().filterNsNode('prop')
                .children().filterNsNode('addressbook-home-set').children().filterNsNode('href').text();
            ...
            var calendar_home = response.children().filterNsNode('propstat').children().filterNsNode('prop')
                .children().filterNsNode('calendar-home-set').children().filterNsNode('href').text();
...
function netLoadResource (inputResource, inputHref, hrefMode, inputResourceIndex, forceLoad, indexR, loadArray) {
    ...
    // console.log("②webdav_func netLoadResource");
    var re = new RegExp('^(https?://)([^/]+)(.*)','i');
    ...
    if (!isAvaible('CalDavZAP') || !globalCalDAVInitLoad || 
        (globalCalDAVInitLoad && typeof inputResource.calendarNo=='undefined' && typeof inputResource.todoNo=='undefined')) {
    ...
    if (typeof globalSubscribedCalendars!='undefined' && globalSubscribedCalendars!=null && typeof inputResource.calendars!='undefined' && 
        inputResource.calendars!=null && inputResource.calendars.length>0) {
        ...
        for (var k=0; k<globalSubscribedCalendars.calendars.length; k++) {
            ...
            if (globalSubscribedCalendars.calendars[k].typeList.indexOf('vevent') != -1) {
                var uidParts  = (uidBase+'/'+globalSubscribedCalendars.calendars[k].href+'/')
                    .match(RegExp('^(https?://)([^@/]+(?:@[^@/]+)?)@(.*)'));
                ...
                var tmp_newobj = {makeLoaded:           toBeLoad, 
                                  typeList:             globalSubscribedCalendars.calendars[k].typeList, 
                                  listType:             'vevent', 
                                  ...
                                  timeOut:              inputResource.timeOut, 
                                  displayvalue:         globalSubscribedCalendars.calendars[k].displayName, 
                                  userAuth:             globalSubscribedCalendars.calendars[k].userAuth, 
                                  resourceIndex:        indexR, 
                                  url:                  baseHref,
                                  accountUID:           origUID1,
                                  href:                 globalSubscribedCalendars.calendars[k].href,
                                  hrefLabel:            globalSubscribedCalendars.hrefLabel,
                                  permissions:          {full: [], read_only: true},
                                  crossDomain:          inputResource.crossDomain,
                                  withCredentials:      inputResource.withCredentials,
                                  interval:             null,
                                  waitInterval:         null,
                                  displayEventsArray:   new Array(),
                                  pastUnloaded:         '',
                                  fcSource:             null,
                                  subscription:         true,
                                  newlyAdded:           toBeLoad,
                                  urlArray:             new Array(),
                                  ignoreAlarms:         globalSubscribedCalendars.calendars[k].ignoreAlarm,
                                  webdav_bind:          false
                                };
                globalResourceCalDAVList.insertResource (tmp_newobj, indexR, true);
            ...
            if (globalSubscribedCalendars.calendars[k].typeList.indexOf('vtodo') != -1) {
                ...
                globalResourceCalDAVList.insertResource (tmp_newobj, indexR, false);
                if (inputResource != undefined) {
    ...
    var settingsXML = '';
    if (inputResource.href.indexOf(globalLoginUsername)!=-1 && 
        inputResource.settingsAccount && globalSettings.settingstype.value!='' && 
        globalSettings.settingstype.value!=null) {

        if ((   globalSettings.settingstype.value == 'addressbook-home-set' && 
                inputResource.abhref == inputHref) || 
            (   globalSettings.settingstype.value == 'calendar-home-set' && 
                inputResource.cahref == inputHref) || 
            (   globalSettings.settingstype.value == 'principal-URL'&& 
                (   (isAvaible('CardDavMATE') && globalCardDAVResourceSync) || 
                    (isAvaible('CalDavZAP') && globalCalDAVResourceSync)))) {
    ...
    function ajaxComplete_Resource (data, textStatus, xml) {
        // 功能：（netLoadResource 函数内）
        // 输入：data, 
        //      textStatus, 
        //      xml
        // 输出：

        // console.log("②webdav_func ajaxComplete_Resource");
        var Rname                 = '';
        inputResource.errorLoaded = false;
        ...
        if (!settingsLoaded && 
            ...
            if ((globalSettings.settingstype.value=='addressbook-home-set' && inputResource.abhref==inputHref) || 
                (globalSettings.settingstype.value=='calendar-home-set' && inputResource.cahref==inputHref)) {
                ...
                if (settings != '') {
                    ...
                    var calSettings = $(xml.responseXML).children().filterNsNode('multistatus').children().filterNsNode('response')
                        .children().filterNsNode('propstat').children().filterNsNode('prop').children().filterNsNode('cal-settings').text();
                    if (calSettings != '') {
                        if (!ignoreServerSettings) {
        ...
        else if (!globalSyncSettingsSave && 
            inputResource.href.indexOf(globalLoginUsername)!=-1 && 
            (   (isAvaible('CardDavMATE') && globalCardDAVResourceSync) || 
                (isAvaible('CalDavZAP') && globalCalDAVResourceSync))) {
        ...
        $(xml.responseXML).children().filterNsNode('multistatus').children().filterNsNode('response').each(function (index, element) {
            $(element).children().filterNsNode('propstat').each(function (pindex, pelement) {
                ...
                if (typeof inputResource != 'undefined' && 
                    ...
                    if ((isAvaible('CalDavZAP') && 
                        resources.children().filterNsNode('resourcetype').children().filterNsNode('calendar').length==1 && 
                        resources.children().filterNsNode('resourcetype').children().filterNsNode('collection').length==1) && 
                        (inputResource.ignoreBound==undefined || !(inputResource.ignoreBound==true && 
                            resources.children().filterNsNode('resourcetype').children().filterNsNode('webdav-binding').length==1))) {
                        ...
                        resources.children().filterNsNode('current-user-privilege-set').children().filterNsNode('privilege').each(function (index, element) {
                        ...
                        if (permissions.length>0 && 
                            ...
                            permissions.indexOf('write-content')==-1) {
                            
                            read_only = true;
                        }
                        else if (inputResource.forceReadOnly!=undefined && 
                            (inputResource.forceReadOnly==true || 
                            inputResource.forceReadOnly instanceof Array)) {
                        ...
                        if (typeList.indexOf('vevent') != -1) {
                            ...
                            resources.children().filterNsNode('acl').children().filterNsNode('ace').each(function(index, el) {
                                if ($(el).children().filterNsNode('principal').children().filterNsNode('authenticated').length > 0 &&
                                    $(el).children().filterNsNode('grant').children().filterNsNode('privilege')
                                        .children().filterNsNode('all').length > 0 &&
                                    $(el).children().filterNsNode('grant').children().filterNsNode('privilege')
                                        .children().filterNsNode('read-current-user-privilege-set').length > 0) {
                            ...
                            var tmp_newobj = {  isSubscribed:       tmp_isSubscribed,
                                                ...
                                                userAuth:           inputResource.userAuth, 
                                                resourceIndex:      indexR, 
                                                url:                baseHref, 
                                                accountUID:         origUID, 
                                                href:               href, 
                                                hrefLabel:          inputResource.hrefLabel, 
                                                permissions:        {full: permissions, read_only: read_only}, 
                                                crossDomain:        inputResource.crossDomain, 
                                                withCredentials:    inputResource.withCredentials, 
                                                interval:           null, 
                                                waitInterval:       null, 
                    ...
                    if (typeof inputResource != 'undefined' && 
                        ...
                        if ((isAvaible('CardDavMATE') && 
                            resources.children().filterNsNode('resourcetype').children().filterNsNode('addressbook').length == 1 && 
                            resources.children().filterNsNode('resourcetype').children().filterNsNode('collection').length == 1) && 
                            (inputResource.ignoreBound == undefined || 
                                !(inputResource.ignoreBound == true && 
                                resources.children().filterNsNode('resourcetype').children().filterNsNode('webdav-binding').length == 1))) {
                            ...
                            if (typeof tmp_lock_support == 'undefined' || 
                                tmp_lock_support.length == undefined || 
                                tmp_lock_support.length == 0) {

                                disableLocking = true;
                            }

                            var href   = $(element).children().filterNsNode('href').text();
                            var tmp_cn = href.match(RegExp('/([^/]+)/?$')); 
                                // collection name

                            var read_only = false;
                            if (((typeof globalDisablePermissionChecking == 'undefined' || 
                                globalDisablePermissionChecking != true) && 
                                    (permissions.length > 0 && 
                                    permissions.indexOf('all') == -1 && 
                                    permissions.indexOf('write') == -1 && 
                                    permissions.indexOf('write-content') == -1)) || 
                                (inputResource.forceReadOnly != undefined && 
                                    (inputResource.forceReadOnly == true || 
                                    inputResource.forceReadOnly instanceof Array && 
                                    inputResource.forceReadOnly.indexOf(tmp_cn[1]) != -1))) {
                            ...
                            if (existingResource != null) {
                                ...
                                if (typeof globalForceSyncURLArray != 'undefined' && 
                                    globalForceSyncURLArray.length > 0 && 
                                    globalForceSyncURLArray.indexOf(existingResource.uid) != -1) { 
                            ...
                            var tmp_newobj = {  makeLoaded:         toBeLoad, 
                                            ...
                                            crossDomain:        inputResource.crossDomain, 
                                                ...
                                                isLoaded:           false, 
                                                newlyAdded:         toBeLoad, 
                                                indexResource:      indexR, 
                                                disableLocking:     disableLocking, 
                                                syncToken:          synctoken, 
                                                oldSyncToken:       oldSyncToken, 
                                                someChanged:        someChanged};

                            globalResourceCardDAVList.insertResource (tmp_newobj, inputResourceIndex);
    ...
    // first try to process the cached data (if cached results are available in the "auth module" response)
    // 第一次尝试获取缓存数据（如果在“auth module”回复中缓存结果是可获取的）
    var tmpCache;
    if (globalXMLCache != null && 
        (tmpCache = globalXMLCache.children('davprincipalcollections[request_url="'+jqueryEscapeSelector(inputHref)+'"]').remove()).length) {

        if (typeof globalDebug!='undefined' && globalDebug instanceof Array && globalDebug.indexOf('cache')!=-1) {
            console.log('DBG Cache OK: '+arguments.callee.name+' url: \''+inputHref+'\': saved one request!');
        }
        ajaxComplete_Resource ('', 'success', {responseXML: tmpCache});
    }
    else {
        if (typeof globalDebug!='undefined' && globalDebug instanceof Array && globalDebug.indexOf('cache')!=-1) {
        ...
        $.ajax({
            ...
            beforeSend: function (req) {
                ...
                req.setRequestHeader('Depth', '1');
                if (globalSettingsSaving!='' || 
                    (   isAvaible('CardDavMATE') && 
                        (   !globalCardDAVInitLoad && 
                            !globalCardDAVResourceSync)) || 
                    (   isAvaible('CalDavZAP') && 
                        (   !globalCalDAVInitLoad && 
                            !globalCalDAVResourceSync)) || 
                    (   isAvaible('Projects') && 
                        isProjectsLoaded)) {
            ...
            data:   '<?xml version="1.0" encoding="utf-8"?>' + 
                    '<D:propfind xmlns:D="DAV:">' + 
                        '<D:prop>' + settingsXML + 
                            ...
                            '<CS:invite xmlns:CS="http://calendarserver.org/ns/"/>' +
                            '<CS:shared-url xmlns:CS="http://calendarserver.org/ns/"/>' +
                            '<D:sync-token/><A:calendar-color xmlns:A="' + 
                                (typeof globalCalendarColorPropertyXmlns!='undefined'&&globalCalendarColorPropertyXmlns!=null&&globalCalendarColorPropertyXmlns!='' ? globalCalendarColorPropertyXmlns : 'http://apple.com/ns/ical/') + '"/>' + 
                            '<I:headervalue xmlns:I="http://inf-it.com/ns/dav/"/><I:addressbook-color xmlns:I="' + 
                                (typeof globalAddrColorPropertyXmlns!='undefined'&&globalAddrColorPropertyXmlns!=null&&globalAddrColorPropertyXmlns!='' ? globalAddrColorPropertyXmlns : 'http://inf-it.com/ns/ab/') + '"/>' + 
                            '<L:supported-calendar-component-set xmlns:L="urn:ietf:params:xml:ns:caldav"/>' + 
                            '<R:max-image-size xmlns:R="urn:ietf:params:xml:ns:carddav"/>' + 
                        '</D:prop>' + 
                    '</D:propfind>',

            dataType: 'xml',
            error: function (objAJAXRequest, strError) {
                console.log("Error: [netLoadResource: 'PROPFIND "+uidFull+"']: code: '"+objAJAXRequest.status+"' status: '"+strError + 
                    "'"+(objAJAXRequest.status==0 ? ' - see https://www.inf-it.com/'+globalAppName.toLowerCase()+'/readme.txt (cross-domain setup)' : ''));
                ...
                if (hrefMode) {
                    ...
                    if ((   (   isAvaible('CardDavMATE') && 
                            ...
                            (   isAvaible('CalDavZAP') && 
                                globalCalDAVInitLoad)) && 
                        indexR == globalAccountSettings.length && allFail) {
            ...
            success: function (data, textStatus, xml) {
                ajaxComplete_Resource (data, textStatus, xml);
                // Cald.mylist.listMy ();
                // updateSettings();
            }
        });
...
function netSaveSettings (inputResource, inputSettings, isFormSave, collectionLoad) {
    // 功能：Save the client settings (stored as DAV property on server)
    //      保存客户端的设定（在服务器上保存）
    // 输入：
    // 输出：
    
    // console.log("②webdav_func netSaveSettings");
    var re          = new RegExp('^(https?://)([^/]+)(.*)', 'i');
    var tmp         = inputResource.href.match(re);
    var baseHref    = tmp[1]+tmp[2];
    var uidBase     = tmp[1]+inputResource.userAuth.userName+'@'+tmp[2];
    ...
    $.ajax({
        ...
        beforeSend: function (req) {
            if (globalSettings.usejqueryauth.value != true && 
                inputResource.userAuth.userName != '' && 
                inputResource.userAuth.userPassword != '') {

                req.setRequestHeader('Authorization',
                                     basicAuth (inputResource.userAuth.userName, inputResource.userAuth.userPassword));
            }
        ...
        data:   '<?xml version="1.0" encoding="utf-8"?>' + 
                '<D:propertyupdate xmlns:D="DAV:">' + 
                    '<D:set>' + 
                        '<D:prop>' + 
                            '<I:settings xmlns:I="http://inf-it.com/ns/dav/">' + 
                                JSON.stringify(serverSettingss) + 
                            '</I:settings>' + 
                        '</D:prop>' + 
                    '</D:set>' + 
                '</D:propertyupdate>',
        ...
        dataType: 'xml',
        error: function (objAJAXRequest, strError) {
            console.log("Error: [netSaveSettings: 'PROPPATCH "+uidFull+"'] code: '"+objAJAXRequest.status+"' status: '"+strError+"'" + 
                (objAJAXRequest.status==0 ? ' (this error code usually means network connection error, or your browser is trying to make a cross domain query, but it is not allowed by the destination server or the browser itself)': ''));

            if (isAvaible('Settings')) {
                show_editor_loader_messageSettings ('message_error', 
                    localization[globalInterfaceLanguage].errSettingsSaved);
            }
...
function deleteVcalendarFromCollection(inputUID, inputForm, putMode) {
    ...
    // console.log("②webdav_func deleteVcalendarFromCollection");
...
function moveVcalendarToCollection (accountUID, inputUID, inputEtag, inputVcalendar, delUID, inputForm, isFormHidden, deleteMode, textArray) {
    ...
    // console.log("②webdav_func moveVcalendarToCollection");
    var resultTimestamp=new Date().getTime();
    if (inputForm=='vtodo')
    ...
    $.ajax({
        ...
        dataType: 'text',
        error: function (objAJAXRequest, strError) {
            console.log("Error: [moveVcalendarToCollection: 'MOVE from: "+put_href+" to: "+dest_href+"']: code: '"+objAJAXRequest.status + 
                "' status: '"+strError+"'" + 
                (objAJAXRequest.status==0 ? ' - see https://www.inf-it.com/'+globalAppName.toLowerCase()+'/readme.txt (cross-domain setup)' : ''));

            switch (objAJAXRequest.status) {
                case 401:
                    show_editor_loader_messageCalendar (inputForm, 'message_error', 
                                                        errBegin.replace('%%', localization[globalInterfaceLanguage].errHttp401));
                    break;
                case 403:
                    show_editor_loader_messageCalendar (inputForm, 'message_error', 
                                                        errBegin.replace('%%', localization[globalInterfaceLanguage].errHttp403));
                    break;
                case 405:
                    show_editor_loader_messageCalendar (inputForm, 'message_error', 
                                                        errBegin.replace('%%', localization[globalInterfaceLanguage].errHttp405));
                    break;
                case 408:
                    show_editor_loader_messageCalendar (inputForm, 'message_error', 
                                                        errBegin.replace('%%', localization[globalInterfaceLanguage].errHttp408));
                    break;
                case 412:
                    show_editor_loader_messageCalendar (inputForm, 'message_error', 
                                                        errBegin.replace('%%', localization[globalInterfaceLanguage].errHttp412));
                    netLoadCalendar (globalResourceCalDAVList.getCollectionByUID(collection_uid), 
                                    [{etag: '', href: put_href_part}], 
                                    (collection.forceSyncPROPFIND==undefined || collection.forceSyncPROPFIND==false ? true : false), 
                                    false, true, true,true, null, null);
                    break;
                case 500:
                    show_editor_loader_messageCalendar (inputForm, 'message_error', 
                                                        errBegin.replace('%%', localization[globalInterfaceLanguage].errHttp500));
                    break;
                default:
                    show_editor_loader_messageCalendar (inputForm, 'message_error', 
                                                        errBegin.replace('%%', localization[globalInterfaceLanguage].errHttpCommon.replace('%%', objAJAXRequest.status)));
                    break;
            }
            if (inputForm=='vtodo')
...
function putVcalendarToCollection(accountUID, inputUID, inputEtag, inputVcalendar, delUID, inputForm, isFormHidden, deleteMode, textArray) {
    ...
    // console.log("②webdav_func putVcalendarToCollection");
    var resultTimestamp = new Date().getTime();
    if (inputForm == 'vtodo') {
        globalTodoLoaderHide = localization[globalInterfaceLanguage].txtAllSavedTodo;
    }
    ...
    $.ajax({
        ...
        dataType: 'text',
        error: function(objAJAXRequest, strError) {
            console.log("Error: [putVcalendarToCollection: 'PUT " + put_href + "']: code: '" + objAJAXRequest.status + "' status: '" + strError + "'" +
                (objAJAXRequest.status == 0 ? ' - see https://www.inf-it.com/' + globalAppName.toLowerCase() + '/readme.txt (cross-domain setup)' : ''));
                ...
                if (inputForm == 'vevent') {
                    ...
                    for (var j = 0; j < resources.length; j++) {
                        if (rid == resources[j].uid) {
                            ...
                            globalEventList.insertEvent(true, resources[j], tmp_inputevent, true, true, false);
                            break;
                        }
                    }
                } 
...
function CalDAVnetLoadCollection (inputCollection, forceLoad, allSyncMode, recursiveIterator, collections) {
    ...
    // console.log("②webdav_func CalDAVnetLoadCollection");
    ...
    if (collections.length > 0) {
        if (inputCollection.uid!=undefined && inputCollection.makeLoaded && 
            inputCollection.newlyAdded && globalSettingsSaving!='') {

            updateMainLoaderText (inputCollection.listType);
        }
        if (inputCollection.uid==undefined || inputCollection.subscription || 
            (!inputCollection.newlyAdded && !inputCollection.syncRequired && !forceLoad && allSyncMode) || 
            (!inputCollection.newlyAdded && !inputCollection.someChanged && !globalCalDAVInitLoad &&allSyncMode) || 
            ((inputCollection.listType=='vtodo' && globalSettings.todopastlimit.value==null) || 
                (inputCollection.listType=='vevent' && 
                    globalSettings.eventstartpastlimit.value==null && 
                    globalSettings.eventstartfuturelimit.value==null)) && 
            !inputCollection.syncRequired) {
            ...
            if (inputCollection.uid!=undefined) {
                if (!inputCollection.syncRequired && globalCalDAVInitLoad) {
                    ...
                    if (((inputCollection.listType=='vtodo'&&globalSettings.todopastlimit.value==null) || 
                            (inputCollection.listType=='vevent'&&globalSettings.eventstartpastlimit.value==null && 
                                globalSettings.eventstartfuturelimit.value==null)) || 
                        (globalLimitLoading=='' && globalLimitTodoLoading=='')) {

                        globalAccountSettings[inputCollection.resourceIndex].todoNo--;
                        if ((globalAccountSettings[inputCollection.resourceIndex].calendarNo==0 && 
                            ...
                            globalOnlyTodoCalendarNumberCount==globalTodoCalendarNumber) {
    ...
    for (var i=0; i<globalAccountSettings.length; i++) {
        if (globalAccountSettings[i].href == resourceCalDAV_href && 
            globalAccountSettings[i].userAuth.userName == resourceCalDAV_user) {
    ...
    if ((globalSettings.eventstartpastlimit.value!=null || globalSettings.eventstartfuturelimit.value!=null ) && 
        !allSyncMode && inputCollection.listType=='vevent') {
    ...
    if (!inputCollection.makeLoaded) {
        ...
        if ((globalLimitLoading=='' && globalLimitTodoLoading=='') || 
            ((inputCollection.listType=='vtodo' && globalSettings.todopastlimit.value==null) || 
                (inputCollection.listType=='vevent' && globalSettings.eventstartpastlimit.value==null && globalSettings.eventstartfuturelimit.value==null))) {
        ...
        if (recursiveIterator>=collections.length && inputCollection.uid!='undefined' && inputCollection.listType=='vevent') {
            ...
            if ((globalSettings.eventstartpastlimit.value==null && globalSettings.eventstartfuturelimit.value == null ) || allSyncMode) {
                ...
                CalDAVnetLoadCollection (globalResourceCalDAVList.TodoCollections[0], forceLoad, allSyncMode, 0, 
                                            globalResourceCalDAVList.TodoCollections);
            }
        }
        else {
            CalDAVnetLoadCollection (collections[recursiveIterator], forceLoad, allSyncMode, recursiveIterator, collections);
    ...
    if (inputCollection.forceSyncPROPFIND!=undefined && inputCollection.forceSyncPROPFIND==true) {
        var requestText =   '<?xml version="1.0" encoding="utf-8"?>' + 
                            '<D:propfind xmlns:D="DAV:">' + 
                                '<D:prop>' + 
                                    '<D:getcontenttype/>' + 
                                    '<D:getetag/>' + 
        ...
        var requestText =   '<?xml version="1.0" encoding="utf-8"?>' + 
                            '<D:sync-collection xmlns:D="DAV:">' + 
                                ...
                                (forceLoad==true || inputCollection.syncToken==undefined || inputCollection.syncToken=='' ? '<D:sync-token/>' : '<D:sync-token>'+inputCollection.syncToken + '</D:sync-token>') + 
                            '</D:sync-collection>';
    }
    else {
        // if inputCollection.forceSyncPROPFIND is undefined or false
        var requestText = '';
            ...
            if (inputCollection.listType=='vevent') {
                ...
                if (!inputCollection.newlyAdded || globalCalDAVInitLoad) {
                    if (globalSettings.eventstartpastlimit.value!=null && (allSyncMode || globalLimitLoading=='past')) {
                        ...
                        if (allSyncMode) {
                            ...
                            pastInterval = ' start="'+$.fullCalendar.formatDate(pastDate ,"yyyyMMdd'T'HHmmss")+'Z" end="' + 
                                            $.fullCalendar.formatDate(globalBeginPast,"yyyyMMdd'T'HHmmss")+'Z"';
                        }
                                        ...
                                        '<L:filter>' + 
                                            '<L:comp-filter name="VCALENDAR">' + 
                                                '<L:comp-filter name="VEVENT">' + 
                                                    '<L:time-range' + (globalLimitLoading=='past' ? pastInterval : futureInterval) + '/>' + 
                                                '</L:comp-filter>' + 
                                            '</L:comp-filter>' + 
                                        '</L:filter>' + 
                                    '</L:calendar-query>';
                    ...
                    requestText =   '<?xml version="1.0" encoding="UTF-8"?>' + 
                                    '<L:calendar-query xmlns:L="urn:ietf:params:xml:ns:caldav">' + 
                                        ...
                                        '<L:filter>' + 
                                            '<L:comp-filter name="VCALENDAR">' + 
                                                '<L:comp-filter name="VEVENT">' + 
                                                    '<L:time-range' + pastInterval + futureInterval + '/>' + 
                                                '</L:comp-filter>' + 
                                            '</L:comp-filter>' + 
                                        '</L:filter>' + 
                                    '</L:calendar-query>';
                ...
                if (!inputCollection.newlyAdded || globalCalDAVInitLoad) {
                    if (globalSettings.todopastlimit.value!=null && (allSyncMode || globalLimitTodoLoading=='pastTodo')) {
                        if (allSyncMode) {
                            ...
                            pastInterval = ' start="'+$.fullCalendar.formatDate(globalLoadedLimitTodo ,"yyyyMMdd'T'HHmmss")+'Z" end="' + 
                                $.fullCalendar.formatDate (
                                    new Date (
                                        new Date (globalLoadedLimitTodo.getTime()).setMonth(globalLoadedLimitTodo.getMonth() + 
                                            globalSettings.todopastlimit.value+1)),
                                    "yyyyMMdd'T'HHmmss"
                                    )+'Z"';
                        }
                    }
                ...
                /*if (!globalSettings.appleremindersmode.value && globalSettings.eventstartfuturelimit.value!=null && (allSyncMode || globalLimitLoading=='futureTodo'))
                {
                    if (allSyncMode)
                        ...
                        futureInterval = ' start="' + 
                            $.fullCalendar.formatDate(new Date(new Date(globalToLoadedLimitTodo.getTime())
                                .setMonth(globalToLoadedLimitTodo.getMonth()-globalSettings.eventstartfuturelimit.value-1)) ,"yyyyMMdd'T'HHmmss")+'Z" end="'+$.fullCalendar.formatDate(globalToLoadedLimitTodo,"yyyyMMdd'T'HHmmss")+'Z"';
                    }
                }*/
                if (!allSyncMode) {
                    requestText =   '<?xml version="1.0" encoding="utf-8"?>' + 
                                    '<L:calendar-query xmlns:L="urn:ietf:params:xml:ns:caldav">' + 
                                        ...
                                        '<L:filter>' + 
                                            '<L:comp-filter name="VCALENDAR">' + 
                                                '<L:comp-filter name="VTODO">' + 
                                                    '<L:time-range' + (globalLimitTodoLoading=='pastTodo' ? pastInterval : futureInterval) + '/>' + 
                                                '</L:comp-filter>' + 
                                            '</L:comp-filter>' + 
                                        '</L:filter>' + 
                                    '</L:calendar-query>';
    ...
    function ajaxComplete_Collection (data, textStatus, xml) {
        // 功能：解析 ics 文件
        // 输入：data, 
        //      textStatus, 
        //      xml
        ...
        // console.log("②webdav_func ajaxComplete_Collection");
        ...
        if (((inputCollection.listType=='vtodo' && globalSettings.todopastlimit.value!=null) || 
            (inputCollection.listType=='vevent' && 
                (globalSettings.eventstartpastlimit.value!=null || globalSettings.eventstartfuturelimit.value!=null)))) {

            var hrefCounter = 0;
            $(xml.responseXML).children().filterNsNode('multistatus').children().filterNsNode(new RegExp('^(sync-)?response$')).children().filterNsNode('href').each(
                function (index, element) {
                    hrefCounter++;
                }
            );
        ...
        $(xml.responseXML).children().filterNsNode('multistatus').children().filterNsNode(new RegExp('^(sync-)?response$')).each(function (index, element) {
                ...
                if (inputCollection.checkContentType != false) {
                    var contenttypeVal = $(element).children().filterNsNode('propstat').children().filterNsNode('prop')
                                            .children().filterNsNode('getcontenttype').text();
                ...
                var result = $(element).find('*').filterNsNode('status').text();    
                    // note for 404 there is no propstat!
                var match  = false;

                if (hrefVal[hrefVal.length-1]!='/') {
                    /* Google CalDAV problem with resource URL if content type checking is disabled */ 
                    if (allowContent == true) {
                        if (result.match(re_found)) {
                            // HTTP OK
                            vcalendarList[vcalendarList.length] = {etag: etagVal, href: hrefVal};
                            match                               = true;
                        }
                    }
                    if (!match && result.match(re_not_found)) {
                        // HTTP Not Found
                        vcalendarList[vcalendarList.length] = {deleted: true, etag: etagVal, href: hrefVal};
                    }
                }
                if ((forceLoad || prevNew) && 
                    ((inputCollection.listType == 'vtodo' && globalSettings.todopastlimit.value != null) || 
                        (inputCollection.listType == 'vevent' && (globalSettings.eventstartpastlimit.value != null || globalSettings.eventstartfuturelimit.value != null)))) {

                    var resultTimestamp = new Date().getTime();
                    if ($(element).children().filterNsNode('propstat').children().filterNsNode('status').text().match(RegExp('200 OK$'))) {
                        // HTTP OK
                        var uid           = inputCollection.uid+hrefVal.replace(vCalendar.pre['hrefValRex'], '');
                        var vcalendar_raw = $(element).children().filterNsNode('propstat').children().filterNsNode('prop')
                                                .children().filterNsNode('calendar-data').text();
                        ...
                        if ((vcalendar_clean==undefined) || ((check=vcalendar_clean.match(vCalendar.pre['vcalendar']))==null)) {
                            if ((check=vcalendar_clean.match(vCalendar.pre['vcalendar'])) == null) {
                                /*--@steve--*/
                                console.log("((check=vcalendar_clean.match(vCalendar.pre['vcalendar']))==null");
                            }
                        ...
                        if ((check=vcalendar_clean.match(vCalendar.pre['vevent'])) != null) {
                            if (inputCollection.typeList.indexOf('vevent') != -1) {
                                var tmp_inputevent = {  threadChange:   '', 
                                                        isRepeat:       false, 
                                                        isDrawn:        false, 
                                                        isTODO:         false, 
                                                        untilDate:      '', 
                                                        ...
                                                        sortkey:        '', 
                                                        timestamp:      resultTimestamp, 
                                                        accountUID:     inputCollection.accountUID, 
                                                        uid:            uid, 
                                                        displayValue:   inputCollection.displayvalue, 
                                                        counter:        0, 
                                                        etag:           etagVal, 
                                                        vcalendar:      vcalendar_clean};

                                globalEventList.insertEvent (false, inputCollection, tmp_inputevent, false, true,true); 
                                    // when the inputMode=='sync' we force reload the vevent/vtodo
                            }
                            else {
                                checkEventLoader (globalResourceCalDAVList.counterList[inputCollection.uid+' '+inputCollection.listType],false);
                            }
                        ...
                        /*else if ((check=vcalendar_clean.match(vCalendar.pre['vtodo'])) != null) {
                            if (inputCollection.typeList.indexOf('vtodo') != -1) {
                                ...
                                globalEventList.insertEvent (false, inputCollection, tmp_inputevent, false, false, true);   
                                    // when the inputMode=='sync' we force reload the vevent/vtodo
                            }
                            else {
                                checkEventLoader (globalResourceCalDAVList.counterList[inputCollection.uid+' '+inputCollection.listType],false);
        ...
        //当设置了仿造，我们需要制作2个ajax过滤请求来读取todo日历
        if (((inputCollection.listType == 'vtodo' && globalSettings.todopastlimit.value != null) || 
            (inputCollection.listType == 'vevent' && 
                (globalSettings.eventstartpastlimit.value != null || globalSettings.eventstartfuturelimit.value != null))) && 
            !isXMLEmpty) {
        ...
        if (((inputCollection.listType == 'vtodo' && globalSettings.todopastlimit.value != null) || 
                (inputCollection.listType == 'vevent' && 
                    (globalSettings.eventstartpastlimit.value != null || globalSettings.eventstartfuturelimit.value != null))) && 
            ((forceLoad && !allSyncMode) || (prevNew && !globalCalDAVInitLoad))) {

            netLoadCalendar (inputCollection, 
                            vcalendarList, 
                            (inputCollection.forceSyncPROPFIND==undefined || inputCollection.forceSyncPROPFIND==false ? true : false), 
                            true, forceLoad,allSyncMode, false, recursiveIterator, collections);
        }
        else {
            netLoadCalendar (inputCollection, 
                            vcalendarList, 
        ...
        vcalendarList = null;
        if (typeof globalParallelAjaxCallCalDAVEnabled != 'undefined' && 
            globalParallelAjaxCallCalDAVEnabled != null && 
            globalParallelAjaxCallCalDAVEnabled) {
    ...
    // first try to process the cached data (if cached results are available in the "auth module" response)
    // 第一次尝试获取缓存数据（如果在 “auth module” 回复中的缓存结果是可获得的）
    var tmpCache;
    var component = '';
    if (inputCollection.listType == 'vtodo') {
        component = 'VTODO';
    ...
    if (globalXMLCache!=null && 
        (   tmpCache = globalXMLCache.children('caldavcalendarquery[request_url="' + jqueryEscapeSelector(inputCollection.url + 
                        inputCollection.href)+'"][request_component="'+component+'"]').remove()).length) {

        if (typeof globalDebug!='undefined' && globalDebug instanceof Array && globalDebug.indexOf('cache')!=-1) {
            console.log('DBG Cache OK: '+arguments.callee.name+' component: '+component+' url: \'' + 
                inputCollection.url+inputCollection.href+'\': saved one request!');
        }
        ajaxComplete_Collection ('', 'success', {responseXML: tmpCache});
    }
    else {
        if (typeof globalDebug!='undefined' && globalDebug instanceof Array && globalDebug.indexOf('cache')!=-1) {
            ...
            error: function (objAJAXRequest, strError) {
                if ((   (   inputCollection.listType == 'vtodo' && 
                        ...
                        (   inputCollection.listType == 'vevent' && 
                            (   globalSettings.eventstartpastlimit.value != null || 
                                globalSettings.eventstartfuturelimit.value != null))) && 
                    objAJAXRequest.responseXML != null && 
                    $(objAJAXRequest.responseXML).children().filterNsNode('C:SUPPORTED-FILTER').length>0) {
                ...
                if ((   objAJAXRequest.status == 400 || /* bad request */ 
                        objAJAXRequest.status == 403 || /* forbidden (for stupid servers) */
                        objAJAXRequest.status == 501 ) && /* unimplemented */
                    inputCollection.forceSyncPROPFIND != true) { /* prevent recursion */
                    ...
                    if ((   globalLimitTodoLoading == '' && 
                            globalLimitLoading == '') || 
                        ((  inputCollection.listType == 'vtodo' && 
                            globalSettings.todopastlimit.value == null) || 
                            (   inputCollection.listType == 'vevent' && 
                                globalSettings.eventstartpastlimit.value == null && 
                                globalSettings.eventstartfuturelimit.value == null))) {
                    ...
                    console.log("Error: [CalDAVnetLoadCollection: '" + 
                        (inputCollection.forceSyncPROPFIND!=undefined && inputCollection.forceSyncPROPFIND==true ? 'PROPFIND' : 'REPORT') + 
                        " "+inputCollection.url+inputCollection.href+"'] code: '"+objAJAXRequest.status+"' status: '"+strError+"'" + 
                        (objAJAXRequest.status==0 ? ' - see https://www.inf-it.com/'+globalAppName.toLowerCase() + '/readme.txt (cross-domain setup)' : ''));
                    
                    return false;
                }
            },
            success:        ajaxComplete_Collection
        });
    }
}

function netLoadCalendar (inputCollection, vcalendarList, syncReportSupport, removeUntouched, forceLoad, allSyncMode, forceCall, recursiveIterator, collections, isInvitation) {
    // 功能：
    // 输入：
    // 输出：

    // console.log("②webdav_func netLoadCalendar");
    ...
    if (!inputCollection.subscription) {
        if (syncReportSupport == true) {
            ...
            // no sync-collection REPORT supported (we need to delete vevents/vtodos by timestamp comparison)
            for (var i=0;i<vcalendarList.length; i++) {
            ...
            if ((check=vcalendar_clean.match(vCalendar.pre['vevent'])) != null) {
                if (inputCollection.typeList.indexOf('vevent') != -1) {
                    var tmp_inputevent = {  threadChange:   '', 
                                            ...
                                            counter:        0, 
                                            etag:           etag, 
                                            vcalendar:      'BEGIN:VCALENDAR'+vcalendar_clean+ 'END:VCALENDAR\r\n'};

                    globalEventList.insertEvent (forceCall, inputCollection, tmp_inputevent, false, true,true); 
                        // when the inputMode=='sync' we force reload the vevent/vtodo
                }
                else {
                    checkEventLoader(globalResourceCalDAVList.counterList[inputCollection.uid+' '+inputCollection.listType],false);
                ...
                if (inputCollection.typeList.indexOf('vtodo') != -1) {
                    ...
                    globalEventList.insertEvent (forceCall, inputCollection, tmp_inputevent, false, false,true);    
                        // when the inputMode=='sync' we force reload the vevent/vtodo
                }
                else {
                    checkEventLoader (globalResourceCalDAVList.counterList[inputCollection.uid+' '+inputCollection.listType],false);
    ...
    multigetData =  '<?xml version="1.0" encoding="utf-8"?>' + 
                    '<L:calendar-multiget xmlns:D="DAV:" xmlns:L="urn:ietf:params:xml:ns:caldav">' + 
                        '<D:prop>' + 
                            '<D:getetag/>' + 
                            '<L:calendar-data/>' + 
                        '</D:prop>' + 
                        '<D:href>' + 
                            vcalendarChangedList.join('</D:href><D:href>') + 
                        '</D:href>' + 
                    '</L:calendar-multiget>';
    ...
    $.ajax({
        ...
        error: function (objAJAXRequest, strError) {
            ...
            console.log("Error: [netLoadCalendar: 'REPORT "+inputCollection.url+inputCollection.href+"'] code: '" + 
                objAJAXRequest.status+"' status: '"+strError+"'" + 
                (objAJAXRequest.status==0 ? ' - see https://www.inf-it.com/'+globalAppName.toLowerCase()+'/readme.txt (cross-domain setup)' : ''));
        ...
        success: function (data, textStatus, xml) {
            ...
            $(xml.responseXML).children().filterNsNode('multistatus').children()
                .filterNsNode('response').children().filterNsNode('href').each(function (index, element) {
            ...
            $(xml.responseXML).children().filterNsNode('multistatus').children().filterNsNode('response').each(function (index, element) {
                if ($(element).children().filterNsNode('propstat').children().filterNsNode('status').text().match(re_found)) {
                    // HTTP OK
                    isXMLEmpty        = false;
                    var etag          = $(element).children().filterNsNode('propstat').children()
                                            .filterNsNode('prop').children().filterNsNode('getetag').text();
                    var uid           = inputCollection.uid+$(element).children()
                                            .filterNsNode('href').text().replace(vCalendar.pre['hrefValRex'], '');
                    var vcalendar_raw = $(element).children().filterNsNode('propstat').children()
                                            .filterNsNode('prop').children().filterNsNode('calendar-data').text();
...
function netLoadCalendarSubscription (inputResource, inputCollection, recursiveIterator, forceLoad, collections) {
    ...
    // console.log("②webdav_func netLoadCalendarSubscription");
    ...
    $.ajax({
        ...
        success: function (data, response, text) {
            ...
            netLoadCalendar (inputCollection, inputCollection.urlArray, 
                            (inputCollection.forceSyncPROPFIND==undefined || inputCollection.forceSyncPROPFIND==false ? true : false), 
                            true, forceLoad, true, false, recursiveIterator, collections);
        }
    });
}
...
function netVersionCheck () {
    ...
    // console.log("②webdav_func netVersionCheck");
    $.ajax({
        type: 'GET',
        ...
        success: function (data, textStatus, xml) {
            ...
            if (current_version_int<version_int)
            {
                ...
                {
                    $('div.update_h').html(localization[globalInterfaceLanguage].updateNotification.replace('%name%',globalAppName)
                                                .replace('%new_ver%','<span id="newversion" class="update_h"></span>')
                                                .replace('%curr_ver%', '<span id="version" class="update_h"></span>')
                                                .replace('%url%', '<span id="homeurl" class="update_h" onclick=""></span>'));
