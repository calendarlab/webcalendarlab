            ...
            if (inputResource.href == resource_href && inputResource.userAuth.userName == resource_user) {
                if (globalSettingsSaving == 'event' && isError) {
                    ...
                    if (isLoaded && collections[i].oldSyncToken == '') {
                        var newObj = $.extend(collections[i], {
                            makeLoaded: true
                        });
...
function unloadCalDAVCollection(unloadArray, isEvent) {
    ...
    if ((globalSettingsSaving == 'event' || globalSettingsSaving == 'todo') &&
        !globalFirstHideLoader) {

        setTimeout(function() {
            hideUnloadCollectionCallback(globalSettingsSaving);
        }, 300);
    }
}
...
function addLoadCalDAVCollection(loadingArray, isEvent) {
    ...
    for (var i = 0; i < collArray.length; i++) {
        if (collArray[i].uid != undefined) {
            ...
            if (loadingArray.indexOf(checkHref) != -1 && !collArray[i].makeLoaded) {
                ...
                var resDOMItem = $('#ResourceCalDAV' + todoString + 'List').find('.myListItem[data-id="' + jqueryEscapeSelector(collArray[i].uid) + '"]');
                ...
                for (var j = 0; j < globalAccountSettings.length; j++) {
                    if (globalAccountSettings[j].href == resourceCalDAV_href &&
                        globalAccountSettings[j].userAuth.userName == resourceCalDAV_user &&
                        globalLoadedPrincipals.indexOf(resourceCalDAV_href) == -1) {
                ...
                collectionChBoxClick(input.get(0), '#ResourceCalDAV' + todoString + 'List', '.resourceCalDAV' + todoString + '_header', '.myListItem', null, false);

                if (window['globalVisibleCalDAV' + todoString + 'Collections'].indexOf(collArray[i].uid) == -1) {
                    window['globalVisibleCalDAV' + todoString + 'Collections']
                        .splice(window['globalVisibleCalDAV' + todoString + 'Collections'].length, 0, collArray[i].uid);
                }
    ...
    for (var i = 0; i < collArray.length; i++) {
        if (collArray[i].uid != undefined) {
            calendarsArray[calendarsArray.length] = {
                displayValue: collArray[i].displayvalue,
                uid: collArray[i].uid,
                permissions_read_only: collArray[i].permissions.read_only,
                makeLoaded: collArray[i].makeLoaded
            };
        }
    }
...
function ResourceCalDAVList() {
    this.collections = new Array(); //集合
    this.TodoCollections = new Array(); //一切集合
    this.counterList = new Array(); //计数器列表
    this.sortedTodoCollections = new Array(); //分类任务集合
    this.sortedCollections = new Array(); //排序的集合
    this.calendarsLoaded = null; //日历加载
    ...
    this.insertResource = function(inputResource, index, isEvent) {
        ...
        for (var i = 0; i < collObject.length; i++) {
            if (collObject[i].uid == inputResource.uid) {
                ...
                // var resDOMItem =
                //  $('#ResourceCalDAV'+todoString+'List')
                //      .find('.resourceCalDAV'+todoString+'_item[data-id="'+jqueryEscapeSelector(inputResource.uid)+'"]');
                var resDOMItem =
                    $('#ResourceCalDAV' + todoString + 'List')
                    .find('.myListItem[data-id="' + jqueryEscapeSelector(inputResource.uid) + '"]');

                if (collObject[i].displayvalue == inputResource.displayvalue &&
                    collObject[i].ecolor == inputResource.ecolor &&
                    collObject[i].isShared == inputResource.isShared &&
                    collObject[i].permissions.read_only == inputResource.permissions.read_only) {
                    // 若是以上信息未被改变则仅仅更新以下信息,增添 ecolor 和 isShared 的校验(删除 headvalue)

                    collObject[i] = $.extend(inputResource, {
                        fcSource: collObject[i].fcSource,
                        syncToken: collObject[i].syncToken,
                        oldSyncToken: collObject[i].oldSyncToken,
                        newlyAdded: collObject[i].newlyAdded,
                        forceSyncPROPFIND: collObject[i].forceSyncPROPFIND
                    });

                    return 0;
                } else {
                    nameChanged = true;
                    $.extend(inputResource, {
                        fcSource: collObject[i].fcSource,
                        syncToken: collObject[i].syncToken,
                        oldSyncToken: collObject[i].oldSyncToken,
                        newlyAdded: collObject[i].newlyAdded,
                    ...
                    makeChecked = resDOMItem.find('input[type=checkbox]')
                        .not('.unloadCheck').prop('checked');
        ...
        if (!globalCalDAVInitLoad && !nameChanged) {
            ...
            if (typeof globalCrossServerSettingsURL != 'undefined' &&
                globalCrossServerSettingsURL != null & globalCrossServerSettingsURL) {
        ...
        var headerObject = {
            headerOnly: true,
            sortkey: this.getSortKey(inputResource, true, index),
            displayvalue: (oldHrefLabel == '%x' ? headerValue.replace(RegExp('^[^#]+#'), '') : headerValue),
            index: 0
        };
        ...
        if (collObject.length > 0) {
            while (low < high) {
                ...
                if (result == -1) {
                    if (insertIndex + 1 == collObject.length - 1 &&
                        typeof collObject[insertIndex + 1] != 'undefined' &&
                        (cmp_str = collObject[insertIndex + 1].sortkey)
                        .customCompare(inputResource.sortkey, globalSortAlphabet, 1, false) == -1) {
                    ...
                    if ((cmp_str = collObject[insertIndex - 1].sortkey)
                        .customCompare(inputResource.sortkey, globalSortAlphabet, 1, false) == -1) {
        ...
        /*var headerMiss = 1;
        for (var i=0; i<collObject.length; i++) {
            if (collObject[i].headerOnly==true &&
                collObject[i].displayvalue==headerObject.displayvalue) {
        ...
        this.counterList[inputResource.uid + ' ' + inputResource.listType] = {
            collectionLength: 0,
            counter: 0,
            uid: inputResource.uid,
            isLoading: false,
            isSaving: false
        };
        ...
        /*if (headerMiss) {
            ...
            if (todoString == '') {
                newElement.find('input[type=checkbox]')
                    .attr('onclick',
                          'resourceChBoxClick (this, \'#\'+$(this).parent().parent().attr(\'id\'), \'' +
                          resHeader+'\', false);if (isCalDAVLoaded && $(this).parent().parent().attr(\'id\')== \'ResourceCalDAV' +
                          todoString+'List\'){$(this).prop(\'checked\')?enableResource($(this).parent()):disableResource($(this).parent());}');
            }
            else {
                newElement.find('input[type=checkbox]')
                    .attr('onclick',
                          'resourceChBoxClick (this, \'#\'+$(this).parent().parent().attr(\'id\'), \'' +
                          resHeader+'\', false);if (isCalDAVLoaded && $(this).parent().parent().attr(\'id\')== \'ResourceCalDAV' +
                          todoString+'List\'){$(this).prop(\'checked\')?enableResourceTodo($(this).parent()):disableResourceTodo($(this).parent());}');
            }

            newElement.css('display', 'none');
            newElement = newElement.parent().html();
            $('#ResourceCalDAV'+todoString+'List')
                .children().eq(insertIndex).after(newElement);
        }*/
        ...
        if (inputResource.isSubscribed) {
            // 这是一个订阅日历
            var newElement = $('#othersListItemTemplate').find('.othersListItem').clone().wrap('<div>');
        } else {
            var newElement = $('#myListItemTemplate').find('.myListItem').clone().wrap('<div>');
        }
        ...
        if (globalDefaultCalendar === null && inputResource.isSubscribed === false) {
            // 默认日历
            newElement.addClass('resourceCalDAV_item_selected');
            globalDefaultCalendar = inputResource;
        }
        ...
        if (inputResource.isSubscribed) {
            ...
            newElement.html(
                ...
                "<input type='checkbox' id=" + inputResource.uid + " class='regular-checkbox'/>" +
                "<label for=" + inputResource.uid + "></label>" +
                "<span class='ListItemTitle'>" + inputResource.displayvalue + "</span>" +
                "<span class='ListItemOwner'>" + inputResource.origin_user + "</span>" +
                "<input type='image' src='images/list_item.png' alt='show/hide calendar options' class='ListItemOption'></input>"
            );

            newElement.find('.ListItemOption')
                .click(function() {
                    // 他人日历列表中点击之后弹出编辑日历对话框
                    if ($('#list_ViewCalendar').css('display') === 'none') {
                        $('#list_ViewCalendar').css({
                            display: 'block',
                        ...
                        // --添加数据：标题、是否共享、颜色--
                        var tmp_uid = $(this).parent().attr('data-id');
                        var tmp_dv = globalResourceCalDAVList.getCollectionByUID(tmp_uid).displayvalue;
                        var tmp_color = globalResourceCalDAVList.getCollectionByUID(tmp_uid).ecolor;
                        var tmp_user = globalResourceCalDAVList.getCollectionByUID(tmp_uid).origin_user;
                        ...
                        $('#list_ViewCalendar .list-color span').each(function() {
                            // 遍历试图匹配日历颜色，
                            // 若是匹配成功则将div移动到对应位置，若是匹配不上，则不操作
                            if (tmp_color === rgbToHex($(this).css('background-color'))) {
                                // 匹配成功
                                $(this).trigger('click');
                            }
                        });
            ...
            $('#listUnsubscribe').click(function() {
                // 取消订阅日历，通过 list_txt 中保存的 data-id 找到对应的资源
                var tmp_uid = $('#list_ViewCalendar .list_txt').attr('data-id');    
                if (tmp_uid === inputResource.uid) {
                    // 防止多次触发...
                    var tmp_url = globalResourceCalDAVList.getCollectionByUID(tmp_uid).origin_address;
                    netUnSubscribeCalendar(tmp_url);
                }
            ...
            newElement.html(
                ...
                "<label for=" + inputResource.uid + "></label>" +
                "<span class='ListItemTitle'>" + inputResource.displayvalue + "</span>" +
                "<span class='ListItemShare'>共享日历</span>" +
                "<input type='image' src='images/list_item.png' alt='show/hide calendar options' class='ListItemOption'></input>"
            );
            if (!inputResource.isShared) {
                // 当前日历设置为不共享
                newElement.find('.ListItemShare').css('visibility', 'hidden');
            }
            ...
            newElement.find('.ListItemOption')
                .click(function() {
                    ...
                    if ($('#list_EditCalendar').css('display') === 'none') {
                        ...
                        $('#list_EditCalendar .list_txt').val(tmp_dv);
                        $('#list_EditCalendar .list_txt').attr('data-id', tmp_uid);

                        $('#list_EditCalendar span.setChoose.active').removeClass('active');
                        if (inputResource.isShared) {
                            // 当前日历设置为共享
                            $('#list_EditCalendar .sharedOpen span').addClass('active');
                        } else {
                            $('#list_EditCalendar .sharedClose span').addClass('active');
                        }

                        $('#list_EditCalendar .list-color span').each(function() {
                            // 遍历试图匹配日历颜色，
                            // 若是匹配成功则将div移动到对应位置，若是匹配不上，则不操作
                            if (tmp_color === rgbToHex($(this).css('background-color'))) {
                                // 匹配成功
        ...
        newElement.find('.ListItemOption')
            .hover(function() {
                // 更换option图标
                newElement.find('.ListItemOption').attr('src', 'images/list_item_hover.png');
            }, function() {
                newElement.find('.ListItemOption').attr('src', 'images/list_item.png');
            });

        newElement.find('input[type=checkbox]').prop('checked', 'true');
        newElement.find('.regular-checkbox + label').css('border-color', inputResource.ecolor);
        newElement.find('.labelafter').css({
            background: inputResource.ecolor,
            display: 'block'
        });
        ...
        if (todoString == '') {
            newElement.find('input[type=checkbox]')
                .attr({
                    'data-id': inputResource.uid,
                    'onclick': 'var evt = arguments[0];evt.stopPropagation();collectionChBoxClick(this, \'#\'+$(this).parent().parent().attr(\'id\'), \'' +
                        resHeader + '\', \'' + resItem + '\', null, false);if (isCalDAVLoaded && $(this).parent().parent().attr(\'id\')== \'ResourceCalDAV' +
                        todoString + 'List\'){$(this).prop(\'checked\')?enableCalendar(\'' + inputResource.uid + '\'):disableCalendar(\'' + inputResource.uid + '\');}'
                });
        } else {
            newElement.find('input[type=checkbox]')
                .attr({
                    ...
                    'onclick': 'var evt = arguments[0];evt.stopPropagation();collectionChBoxClick(this, \'#\'+$(this).parent().parent().attr(\'id\'), \'' +
                        resHeader + '\', \'' + resItem + '\', null, false);if (isCalDAVLoaded && $(this).parent().parent().attr(\'id\')== \'ResourceCalDAV' +
                        todoString + 'List\'){$(this).prop(\'checked\')?enableCalendarTodo(\'' + inputResource.uid + '\'):disableCalendarTodo(\'' + inputResource.uid + '\');}'
                });
        }

        newElement.find('.regular-checkbox + label, .labelafter').click(function(event) {
            /* 点击label或span：模拟点击checkbox，并且模拟界面 */
            newElement.find('input[type=checkbox]').click();

            if (newElement.find('input[type=checkbox]').prop('checked')) {
                newElement.find('.labelafter').css('display', 'block');
            } else {
                newElement.find('.labelafter').css('display', 'none');
            }
        });

        newElement.click(function(e) {
            if ((newElement.hasClass('resourceCalDAV_item') && globalEventCollectionsLoading) ||
                (newElement.hasClass('resourceCalDAVTODO_item') && globalTodoCollectionsLoading)) {

                return true;
            }
            if (e.shiftKey) {
                if (isEvent) {
                    enableOne(newElement.attr('data-id'));
                } else {
                    enableOneTodo(newElement.attr('data-id'));
                }
            }

            /*if (newElement.hasClass('resourceCalDAV_item_selected')) {
                newElement.removeClass('resourceCalDAV_item_selected')
            }
            else {
                newElement.addClass('resourceCalDAV_item_selected');
            }*/
        ...
        // if (typeof globalCalendarColorPropertyXmlns=='undefined' ||
        //  globalCalendarColorPropertyXmlns==null ||
        //  globalCalendarColorPropertyXmlns==='' ||
        //  globalCalendarColorPropertyXmlns!==false) {
        ...
        if (inputResource.isSubscribed) {
            // 将他人日历插在最后
            $('#ResourceCalDAV' + todoString + 'List').children().eq($('#ResourceCalDAV' + todoString + 'List').children().length - 1).after(newElement);
        } else {
            // 将我的日历插入在最上方
            $('#ResourceCalDAV' + todoString + 'List').children().eq(0).after(newElement);
    ...
    this.removeOldResources = function(inputUidBase, inputTimestamp) {
        ...
        for (var i = this.collections.length - 1; i >= 0; i--) {
            if (this.collections[i] != undefined &&
                !this.collections[i].subscription &&
                this.collections[i].timestamp != undefined &&
                this.collections[i].uid.indexOf(inputUidBase) == 0 &&
                this.collections[i].timestamp < inputTimestamp) {
                ...
                var item = $('#ResourceCalDAVList').find('div[data-id^="' + jqueryEscapeSelector(this.collections[i].uid) + '"]');
                ...
                if ((this.collections[i] == undefined || this.collections[i].headerOnly == true) &&
                    this.collections[i - 1].headerOnly == true) {
                    ...
                    if (firstVisibleCollection == null) {
                        ...
                        var triggerInput = $('#ResourceCalDAVList').find('.myListItem[data-id^="' + jqueryEscapeSelector(firstVisibleCollection.uid) + '"]').find('input[type=checkbox]');
                        // collectionChBoxClick(triggerInput.get(0), '#ResourceCalDAVList', '.resourceCalDAV_header', '.resourceCalDAV_item', null, false);
                        collectionChBoxClick(triggerInput.get(0), '#ResourceCalDAVList', '.resourceCalDAV_header', '.myListItem', null, false);
                    }
                }
            }
        }
        ...
        for (var i = this.TodoCollections.length - 1; i >= 0; i--) {
            if (this.TodoCollections[i] != undefined &&
                !this.TodoCollections[i].subscription &&
                this.TodoCollections[i].timestamp != undefined &&
                this.TodoCollections[i].uid.indexOf(inputUidBase) == 0 &&
                this.TodoCollections[i].timestamp < inputTimestamp) {
        ...
        if (isEvent) {
            ...
            for (var i = this.TodoCollections.length - 1; i >= 0; i--) {
                if (this.TodoCollections[i].uid == inputUid) {
                    ...
                    // if next item is undefined or it is a header and the previous item is header delete it
                    if ((this.TodoCollections[i] == undefined || this.TodoCollections[i].headerOnly == true) &&
                        i > 0 &&
                        this.TodoCollections[i - 1].headerOnly == true) {
    ...
    this.getResources = function() { //得到资源
        return this.collections;
    };

    this.getSyncResourceArray = function() { //获取同步资源数组
