 ...
 ******************/
 ...
 (function($, undefined) {
    var defaults = {
        ...
        titleFormat: {
            month:      'MMMM yyyy',
            multiWeek:  "MMM d[ yyyy]{ 'â€?[ MMM] d yyyy}",
            week:       "MMM d[ yyyy]{ 'â€?[ MMM] d yyyy}",
            day:        'dddd, MMM d, yyyy',
            list:       'MMM d, yyyy',
            table:      "MMM d[ yyyy]{ 'â€?[ MMM] d yyyy}",
            todo:       "MMM d[ yyyy]{ 'â€?[ MMM] d yyyy}",
        },
        columnFormat: {
            month:      'ddd',
            multiWeek:  'ddd',
        ...
        timeFormat: {                                               // for event elements
            '':             'h(:mm)t',                              // default
            agenda:         'h:mm{ â€?h:mm}',                       //agenda views
            list:           'hh:mm{ â€?hh:mm}',                     //list and table views
            listFull:       'hh:mm M d yyyy{ â€?hh:mm M d yyyy}',   //list and table views for events that span multiple days
            listFullAllDay: 'M d yyyy{ â€?M d yyyy}',               //list and table views for allday events that span multiple days
        },
        ...
        showWeekNumbers:        true,   
            /*是否在日历中显示周次(一年中的第几周)，如果设置为true，
            则会在月视图的左侧、周视图和日视图的左上角显示周数。*/
        multiWeekSize:          3,
    ...
    function setDefaults(d) {
        // console.log("③fullcalendar_func setDefaults");
    ...
    function Calendar(element, options, eventSources) {
        ...
        function setOptions(newOptions) {
            ...
            $.each(newOptions, function (key,value){
                ...
                if (key=='firstDay' || key=='timeFormat') {
                    ...
                    for (var view in t.viewInstances){
                        // console.log("t.viewInstances[view]: ", t.viewInstances[view]);
                        // console.log("'set'+key.charAt(0).toUpperCase()+key.slice(1): ", 'set'+key.charAt(0).toUpperCase()+key.slice(1));
        ...
        function changeView (newViewName) {
            if (!currentView || newViewName != currentView.name) {
                ...
                if (newViewName == 'agendaDay') {
                    ...
                    if (addedView) {
                        ...
                        addedView = t.viewInstances['table'] = new fcViews['table'](
                            addedNewViewElement = addedAbsoluteViewElement = $("<div class='fc-view fc-view-table' style='position:absolute'/>").appendTo(content),
                            t // the calendar object
                        );
                ...
                $('div.fc-view-table').css('overflow', 'visible');
        ...
        function renderView (inc) {
            // console.log("function renderView");
            hideEventPopup();               // 消除事件详情依然显示的bug
        ...
        function updateSize() {
            ...
            if (elementVisible()) {
                ...
                if (currentView.name != 'todo') {
                    ...
                    // console.log("if (currentView.name != 'todo')");
                }
                currentView.sizeDirty = false;
            }
        }
            ...
            if (currentView.addedView) {
                if (currentView.addedView.getDaySegmentContainer().find('.fc-today').length > 0) {
                    if (new Date().getDate() == 1) {
                        if (currentView.addedView.name == 'table') {
                            $('div.fc-list-content').mCustomScrollbar("scrollTo", "top");
                        } 
                        else {
                        ...
                        if (currentView.addedView.name == 'table') {
                            // 日视图中的特殊处理，修正了offset
                            offset = $('.fc-view-table tbody.fc-today td').position().top;
                            $('div.fc-list-content').mCustomScrollbar("scrollTo", top + offset);
                        } 
                        else {
    ...
    function Header(calendar, options) {
        ...
        function render() {
            ...
            if (sections) {
                //@steve...Toggle button
                var tmp_Left = $("<td style='width: 8.5%'/>").append(
                    "<input id='ResourceCalDAVToggle' type='image' src='images/btn_res_toggle.png' alt='Show/Hide Resources' />" +
                    "<input id='ButtonMsg' type='image' src='images/btn_msg.png' alt='Show Msg' />" + 
                    "<div id='newInvitationNum'></div>"
                );

                //@steve...search form
                var tmp_Search = $("<td/>").append(
                    "<div id='searchForm'>" + 
                    "<img id='searchIcon' alt='Search Form' src='images/search.png' style='position: inline; margin-top: 4px; margin-left: 8px; vertical-align: top;' />" + 
                    "<div class='searchContainer'>" + 
                    // "<input type='text' value='' placeholder='Search' data-type='PH_CalDAVsearch' id='searchInput' style='margin-top: 5px; vertical-align: top;' />" + "</div>" +
                    "<input type='text' value='' data-type='PH_CalDAVsearch' id='searchInput' />" + "</div>" +
                    "<img alt='Search Reset'  id='reserButton' onclick=\"$('#searchInput').val('');$('#searchInput').keyup();\" src='images/reset_contact.png'  />" + "</div>"
                );

                var tmp_View_Mod = $("<td style='width: 77.5%;' />").append(
                    "<div id='mwd_box' style='height: 30px;width: 283px; background: url(images/view_mod_bg.png) no-repeat; margin: 0 auto;'>" + 
                    '&nbsp' +
                    "</div>"
                );

                //@steve...logout
                // var tmp_logout = $("<td/>").append(
                //  "<img id='eventFormShower' src='images/banner_logout.svg' \
                //  alt='logout' onclick='logout();' />"
                // );
                
                element = $("<table class='fc-header' style='width:100%;height:100%;'/>")
                    .append(
                        $("<tr/>")
                        .append(tmp_Left)       
                ...
                // -----event binding--------
                tmp_Left.find('#ResourceCalDAVToggle')
                    .hover(function() {
                        $('#ResourceCalDAVToggle').attr('src', 'images/btn_res_toggle_hover.png');
                    }, function() {
                        $('#ResourceCalDAVToggle').attr('src', 'images/btn_res_toggle.png');
                    })
                    .click(function() {
                        // 显示、隐藏日历列表
                        ...
                        var col1 = isResourceVisible ? 0 : 249;
                        var col2 = isResourceVisible ? 0 : 250;
                        var col3 = isResourceVisible ? 0 : 250;
                        ...
                        $('#main_h, #main').animate({left: col2}, transSpeedResource).promise().done(function(){
                            $(window).resize();
                        });
                    }
                );

                tmp_Left.find('#ButtonMsg')
                    .hover( function() {
                            $('#ButtonMsg').attr('src', 'images/btn_msg_hover.png');
                        }, function() {
                            $('#ButtonMsg').attr('src', 'images/btn_msg.png');
                        }
                    )
                    .click(function () {
                        if ($('#invitationEventBox').css('display') === 'none') {
                            $('#invitationEventBox').fadeIn('slow');
                            $('.ezz').css('z-index', 25);
                        } 
                    });

                tmp_Search.bind('keyup change', function() {
                    if ($('#searchInput').val() != ''){
                        $('#reserButton').css('visibility', 'visible');
                        $('#searchIcon').attr('src', 'images/search_change.png');
                        $('#searchForm').css('border-color', '#4bce0d');
                    }
                    else {
                        $('#reserButton').css('visibility', 'hidden');
                        $('#searchIcon').attr('src', 'images/search.png');
                        $('#searchForm').css('border-color', '#c5c5c5');
                    }
                });
                return element;
            }
        ...
        function renderSection (position) {
            if (position == 'right') {
                var tmp_e = $("<td class='fc-header-" + position + "'/>");
                var e     = $("<div id='mwd_box' style='height:26px; width:281px; margin:12px auto 0; border:2px solid #4bce0d; border-radius:15px;'/>");
            } 
            else {
                                ...
                                var button = $(
                                    "<span class='fc-button fc-button-" + buttonName + " " + tm + "-state-default'>" +
                                        "<span class='fc-button-content'>" +
                                            (icon ? "<img src='images/btn_arrow_" + icon + ".png'/>" :
                                                text
                                            ) +
                                        "</span>" +
                                    "</span>"
                                ...
                                if (button) {
                                    button
                                        ...
                                        .hover(
                                            function() {
                                                if (buttonName == 'today') {
                                                    button.find('.fc-button-content').css('color', '#378338');
                                                } 
                                                else if (buttonName == 'prev') {
                                                    button.find('img').attr('src', 'images/btn_arrow_prev_hover.png');
                                                } 
                                                else if (buttonName == 'next') {
                                                    button.find('img').attr('src', 'images/btn_arrow_next_hover.png');
                                                } 
                                                else {
                                                    button
                                                        .not('.' + tm + '-state-active')
                                            ...
                                            function() {
                                                if (buttonName == 'today') {
                                                    button.find('.fc-button-content').css('color', '#4bce0d');
                                                } 
                                                else if (buttonName == 'prev') {
                                                    button.find('img').attr('src', 'images/btn_arrow_prev.png');
                                                } 
                                                else if (buttonName == 'next') {
                                                    button.find('img').attr('src', 'images/btn_arrow_next.png');
                                                } 
                                                else {
                                                    button
        ...
        function updateEvent(event) { // update an existing event
            var i, len          = cache.length, e,
                ...
                endDelta        = event.end ? (event.end - (event._end || defaultEventEnd(event)))  : 0;
                    /*event._end would be null if event.end was null and event was just resized*/
    ...
    function addYears(d, n, keepTime) {
        // console.log("③fullcalendar_func addYears");
    ...
    function addMonths(d, n, keepTime) { 
        // console.log("③fullcalendar_func addMonths");
    ...
    function addDays(d, n, keepTime) { 
        ...
        // console.log("③fullcalendar_func addDays");
        if (+d) {
            var dd = d.getDate() + n,
            check = cloneDate(d);
    ...
    function fixDate(d, check) { 
        ...
        // console.log("③fullcalendar_func fixDate");
    ...
    function addMinutes(d, n) {
        // console.log("③fullcalendar_func addMinutes");
    ...
    function clearTime (d) {
        ...
        d.setMilliseconds(0);
        return d;
        // console.log("③fullcalendar_func clearTime");
    }
    ...
    function cloneDate(d, dontKeepTime) {
        ...
        return new Date(+d);
        // console.log("③fullcalendar_func cloneDate");
    }
    ...
    function skipWeekend(date, inc, excl) {
        // console.log("③fullcalendar_func skipWeekend");
        inc = inc || 1;
    ...
    function dayDiff(d1, d2) { 
        // d1 - d2
        // console.log("③fullcalendar_func dayDiff");
        return Math.round((cloneDate(d1, true) - cloneDate(d2, true)) / DAY_MS);
    }

    function minDiff(d1, d2) {
        // d1 - d2
        // console.log("③fullcalendar_func minDiff");
        return Math.round((cloneDate(d1, false) - cloneDate(d2, false)) / MINUTE_MS);
    }

    function setYMD(date, y, m, d) {
        // console.log("③fullcalendar_func setYMD");
    ...
    function formatDate(date, format, options) {
        // console.log("③fullcalendar_func formatDate");
    ...
    function exclEndDay(event) {
        // console.log("③fullcalendar_func exclEndDay");
        if (event.end) {
    ...
    function _exclEndDay(end, allDay) {
        // console.log("③fullcalendar_func _exclEndDay");
        end = cloneDate(end);
        return allDay || end.getHours() || end.getMinutes() ? addDays(end, 1) : clearTime(end);
    }

    function segCmp(a, b) {
        // console.log("③fullcalendar_func segCmp");
        return (b.msLength - a.msLength) * 100 + (a.event.start - b.event.start);
    }

    function segsCollide(seg1, seg2) {
        // console.log("③fullcalendar_func segsCollide");
    ...
    function sliceSegs(events, visEventEnds, start, end) {
        // console.log("③fullcalendar_func sliceSegs");
        var segs = [],
    ...
    function stackSegs(segs) {
        // console.log("③fullcalendar_func stackSegs");
        var levels = [],
    ...
    function lazySegBind (container, segs, bindHandlers) {
        // console.log("③fullcalendar_func lazySegBind");
    ...
    function setOuterWidth (element, width, includeMargins) {
        // console.log("③fullcalendar_func setOuterWidth");
    ...
    function setOuterHeight (element, height, includeMargins) {
        // console.log("③fullcalendar_func setOuterHeight");
        for (var i=0, e; i<element.length; i++) {
            e = $(element[i]);
    ...
    function hsides (element, includeMargins) {
        // console.log("③fullcalendar_func hsides");
        return hpadding(element) + hborders(element) + (includeMargins ? hmargins(element) : 0);
    }

    function hpadding (element) {
        // console.log("③fullcalendar_func hpadding");
        return (parseFloat($.css(element[0], 'paddingLeft', true)) || 0) +
        (parseFloat($.css(element[0], 'paddingRight', true)) || 0);
    }

    function hmargins (element) {
        // console.log("③fullcalendar_func hmargins");
        return (parseFloat($.css(element[0], 'marginLeft', true)) || 0) +
        (parseFloat($.css(element[0], 'marginRight', true)) || 0);
    }

    function hborders (element) {
        // console.log("③fullcalendar_func hborders");
        return (parseFloat($.css(element[0], 'borderLeftWidth', true)) || 0) +
        (parseFloat($.css(element[0], 'borderRightWidth', true)) || 0);
    }

    function vsides (element, includeMargins) {
        // console.log("③fullcalendar_func vsides");
        return vpadding(element) +  vborders(element) + (includeMargins ? vmargins(element) : 0);
    }

    function vpadding (element) {
        // console.log("③fullcalendar_func vpadding");
        return (parseFloat($.css(element[0], 'paddingTop', true)) || 0) +
        (parseFloat($.css(element[0], 'paddingBottom', true)) || 0);
    }

    function vmargins (element) {
        // console.log("③fullcalendar_func vmargins");
        return (parseFloat($.css(element[0], 'marginTop', true)) || 0) +
        (parseFloat($.css(element[0], 'marginBottom', true)) || 0);
    }

    function vborders (element) {
        // console.log("③fullcalendar_func vborders");
        return (parseFloat($.css(element[0], 'borderTopWidth', true)) || 0) +
        (parseFloat($.css(element[0], 'borderBottomWidth', true)) || 0);
    }

    function setMinHeight (element, height) {
        // console.log("③fullcalendar_func setMinHeight");
    ...
    function setDayID (cell, date, opt) {
        cell.each(function (i, _cell) {
            _cell.className = _cell.className.replace(/^fc-\w*( fc-weekend-day)?/, 'fc-' + dayIDs[date.getDay()] + 
                (opt('weekendDays').length>0 && opt('weekendDays').indexOf(date.getDay())!=-1 ? ' fc-weekend-day' : ''));
    ...
    function MonthView(element, calendar) {
        // console.log("③fullcalendar_func MonthView");
        var t = this;
    ...
    function BasicWeekView(element, calendar) {
        // console.log("③fullcalendar_func BasicWeekView");
        var t = this;
    ...
    function BasicDayView(element, calendar) {
        // console.log("③fullcalendar_func BasicDayView");
        var t = this;
    ...
    function BasicView(element, calendar, viewName) {
        // console.log("③fullcalendar_func BasicView");
        var t = this;
        ...
        function buildSkeleton(maxRowCnt, showNumbers) {
            ...
            for (i=0; i<colCnt; i++) {
                s += "<th class='fc- " + headerClass + 
                    "'><span style='margin-left:9px'></span>"; // need fc- for setDayID
            }
                    ...
                    "<td class='fc- " + contentClass + " fc-day" + (i*colCnt+j) + "'>" + // need fc- for setDayID
                        "<div>" +
                            "<div class='fc-day-header'>" +
                                "<img src='images/circle.png' style='visibility:hidden; margin: 4px 0 0 9px;'/>" + 
            ...
            dayBind(bodyCells);
            daySegmentContainer = $("<div style='position:absolute;z-index:8;top:0;left:0'/>")
                .appendTo(element);
                // 月视图中各个事件的容器

            daySeparateContainer = $("<div style='position:absolute; width:0; top:36px; height:0;'/>")
                .appendTo(element);
                // 月视图中放置分割线的容器
        }

        function setDaySeparateMonth () {
            // 设置月视图中间断线效果...
            var dayseparate = daySeparateContainer.children('.fc-day-separate');
            var tmp_left = 0;
            var tmp_num = 0;

            if ( dayseparate.length == 0 ) { 
                for ( var i = 0; i < 7; i++ ) {
                    // 每天添加两个，放置在左右
                    dayseparate = $('<div>').addClass('fc-day-separate').appendTo(daySeparateContainer);
                    dayseparate = $('<div>').addClass('fc-day-separate').appendTo(daySeparateContainer);
                }
            }
            
            daySeparateContainer.find('.fc-day-separate').each(function (index, el) {
                // 设定每天分割线的css：1. 背景色和div.fc-view-month下的td.fc-day+i保持一致；
                //                    2. 左右位置同上，根据奇偶确定是否加上td的一部分宽度

                $(el).css('backgroundColor', $('div.fc-view-month td.fc-day' + tmp_num).css('backgroundColor') );               
                // $(el).css('height', $('div.fc-view-month tbody').css('height'));                
                $(el).css('height', $('#main').css('height'));                

                if ( index % 2 == 0 ) {
                    // 偶数，放置在左侧
                    $(el).css('left', tmp_left + 'px');
                }
                else {
                    // 奇数，放置在右侧
                    tmp_left += parseInt( $('div.fc-view-month td.fc-day' + tmp_num).css('width') );
                    // console.log("parseInt( $('div.fc-view-month td.fc-day'"+ tmp_num +").css('width') ): ", parseInt( $('div.fc-view-month td.fc-day' + tmp_num).css('width') ));
                    var odd_left = tmp_left - 9;

                    $(el).css('left', odd_left+'px');
                    tmp_num += 1;
                }
            }); 
        }
        ...
        function updateCells (firstTime) {
            ...
            if ( dowDirty ) {
                headCells.each(function (i, _cell) {
                    cell = $(_cell);
                    date = indexDate(i);
                    cell.find('span').html(formatDate(date, colFormat));
            ...
            bodyCells.each(function (i, _cell) {
                ...
                if ( +date == +today ) {
                    ...
                    cell.find('img').css('visibility', 'visible');
                }
                else {
                ...
                if (parseInt(date.getDate()) >= 10 ) {
                    // 当前日期为两位数，改变偏移量...
                    cell.find('div.fc-day-number').css('margin-left', '-32px');
                }
                // else {
                //     // 当前日期为两位数，改变偏移量...
                //     cell.find('div.fc-day-number').css('margin-left', '-26px');
                // }
        ...
        function reportDayClick(date, allDay, ev) {
            ...
            console.log("func reportDayClick");
        ...
        function colContentLeft(col) {
            // console.log("colContentPositions.left(col): ", colContentPositions.left(col));
    ...
    function BasicEventRenderer() {
        // console.log("③fullcalendar_func BasicEventRenderer");
        var t = this;
        ...
        function draggableDayEvent (event, eventElement) {
            ...
            var dayDelta;
            // console.log("func draggableDayEvent");
    ...
    function AgendaWeekView(element, calendar) {
        // console.log("③fullcalendar_func AgendaWeekView");
        var t = this;
    ...
    function AgendaDayView(element, calendar) {
        // console.log("③fullcalendar_func AgendaDayView");
        var t = this;
    ...
    function AgendaView (element, calendar, viewName) {
        // console.log("③fullcalendar_func AgendaView");
        var t = this;
        ...
        function buildSkeleton() {
            ...
            for (i=0; i<colCnt; i++) {
                ...
                "'><div class='fc-header-number'>" + 
                "<span class='fc-number-bgc'/>" + 
                "<div class='fc-number-content'></div>" + "</div>" + 
                    "<div class='fc-header-ddd'></div></th>"; // fc- needed for setDayID
                    //--@steve--
            }
            ...
            slotLayer = $("<div style='position:absolute;z-index:2;left:0;width:100%'/>")
                .appendTo(element);

            if (opt('allDaySlot')) {
                dayScroller = $("<div style='position:absolute;width:100%;overflow-x:hidden;overflow-y:auto;'/>").appendTo(slotLayer);
                dayContent = $("<div style='position:relative;width:100%;overflow:hidden;min-height:55px'/>").appendTo(dayScroller);
                ...
                "<td>" +
                "<div class='fc-day-content'><div style='position:relative;min-height:55px'/></div>" +
                "</td>" +
                "</tr>" +
                "</table>";
            ...
            slotBorder = slotTable.find('th');
            slotBorder.css('border-top-style', 'hidden');
            axisFirstCells = axisFirstCells.add(slotTable.find('th:first'));

            // if (colCnt == 1) {
            //  // 当前为日视图...则不显示<thead>...
            //  dayHead.css('visibility', 'hidden');
            //  $('.fc-todaytimeline').css('display', 'none');
            //  console.log("$('.fc-todaytimeline'): ", $(".fc-todaytimeline").css('visibility'));
            // }

            // slotScroller.mCustomScrollbar({
            //  // 周视图的滚动条
            //  theme: 'minimal-dark',
        ...
        function updateCells() {
            ...
            for (i=0; i<colCnt; i++) {
                ...
                if (parseInt(date.getDate()) >= 10 ) {
                    // 当前日期为两位数，改变偏移量...
                    headCell.find('.fc-number-content').css('left', '7px');
                }
                else {
                    headCell.find('.fc-number-content').css('left', '');
                }
                headCell.find('.fc-number-content').html(date.getDate());
                headCell.find('div:last').html(formatDate(date, colFormat));
                // ----------
                ...
                if (+date == +today) {
                    ...
                    //--@steve--
                    headCell.find('span').css('visibility','visible');
                    //-----------
                }
                else {
                    ...
                    //--@steve--
                    headCell.find('span').css('visibility','hidden');
                    //---------------
                }
            }
        }
        ...
        function setHeight (height, dateChanged) {
            ...
            // allDayWidth = $(.fc-agenda-axis fc-widget-header fc-first).outerwidth();     //@steve
            if (opt('allDaySlot')) {
                divider.css('position', 'relative');
                divider.css('top', allDayHeight);
                divider.css('left', 50);    //@steve
                ...
                axisFirstCells
                    ...
                    .each(function (i, _cell) {
                        axisWidth = Math.max(axisWidth, $(_cell).outerWidth());
                            // console.log("outerWidth is ",axisWidth);
                    }),
                axisWidth
            );
        ...
        function resetScroll() {
            ...
            function scroll() {
                ...
                slotScroller.mCustomScrollbar("scrollTo", top);
            }
            scroll();
        ...
        function afterShow() {
            ...
            slotScroller.mCustomScrollbar("scrollTo", savedScrollTop);
        }
    ...
    function AgendaEventRenderer () {
        // console.log("③fullcalendar_func AgendaEventRenderer");
        var t = this;
        ...
        function setTimeIndicator() {
            ...
            var curtime = container.children('.fc-timeline-curtime');
            var todaytimeline = container.children('.fc-todaytimeline');
            ...
            if (curtime.length==0 || todaytimeline.length==0) { 
                todaytimeline = $('<hr>').addClass('fc-todaytimeline').appendTo(container);
                $('div.fc-view-agendaDay').find('.fc-todaytimeline').css('visibility', 'hidden');
                    // 在日视图中不显示周视图中今天的加粗线...
                var tmp_mins = parseInt(cur_time.getMinutes())<10 ? "0"+String(cur_time.getMinutes()) : cur_time.getMinutes();
                curtime = $('<div>')
                            .addClass('fc-timeline-curtime').appendTo(container)
                            .html(cur_time.getHours()+":"+tmp_mins);
            }

            if (curtime.length != 0) {
                var tmp_mins = parseInt(cur_time.getMinutes())<10 ? "0"+String(cur_time.getMinutes()) : cur_time.getMinutes();
                curtime.html(cur_time.getHours() + ":" + tmp_mins);
            }
            ...
            if (daycol.length > 0) {
                ...
                curtime.hide();
                todaytimeline.hide();
                container.find('.fc-axis-absolute').each(function (index, el) {
            ...
            curtime.css('top', Math.floor(container.height() * percents - 1) -12 + 'px');
            todaytimeline.css('top', Math.floor(container.height() * percents - 3) + 'px');
            todaytimeline.css('width', $('.fc-mon').css('width'));
            $("table[class='fc-agenda-days fc-border-separate'] thead tr[class='fc-first fc-last'] th").each(function (index, el) {
                if ($(el).hasClass('fc-today')) {
                    //找到今天，将相应的宽度赋值给todaytimeline
                    var tmp_width = parseInt($("th[class='fc-agenda-axis fc-widget-header fc-first']").css('width')) + parseInt($('.fc-mon').css('width'))*(index-1);
                    todaytimeline.css('left', tmp_width+'px');
                    return false;   //跳出each循环
                }
            });

            // @steve 左侧的小时显示（未调试显示分钟的情况...）
            var timeaxis = container.children('.fc-axis-absolute');
            d = zeroDate();
            addMinutes(d, opt('slotMinutes')*2);

            if (timeaxis.length == 0) {
                for (var i = 1; i < 24; i++) {
                    if (i != 12) {
                        timeaxis = $('<span>')
                            .addClass('fc-axis-'+i+' fc-axis-absolute')
                            .html(formatDate(d, opt('axisFormat')))
                            .appendTo(container);
                    } 
                    else {
                        timeaxis = $('<span>')
                    ...
                    var ratio = i / 24;
                    timeaxis.css('top', Math.floor(container.height() * ratio - 11) + 'px');
                    addMinutes(d, opt('slotMinutes')*2);
                }
            }

            var curtime_th = parseInt(curtime.css('top')) + parseInt(curtime.css('height'));
            container.find('.fc-axis-absolute').each(function (index, el) {
                var el_th = parseInt($(el).css('top')) + parseInt($(el).css('height'));
                if (curtime_th>parseInt($(el).css('top')) && parseInt(curtime.css('top'))<el_th) {
                    //当前时间已遮挡显示的小时，将小时隐藏...
        ...
        function setDaySeparate() {
            // 绘制周视图中的间断线效果
            var container = getBodyContent();
            var dayseparate = container.children('.fc-day-separate');
            ...
            var tmp_left =  parseInt( $("div.fc-view-agendaWeek th[class='fc-agenda-axis fc-widget-header fc-first']" ).css('width') );
            var tmp_num = 0;

            container.find('.fc-day-separate').each(function (index, el) {
                // 设定每天分割线的css：1. 背景色和div.fc-view-agendaWeek下的td.fc-col+i保持一致；
                //                    2. 左右位置同上，根据奇偶确定是否加上td的一部分宽度
                $(el).css( 'backgroundColor', $('div.fc-view-agendaWeek td.fc-col' + tmp_num).css('backgroundColor') );
                ...
                if ( index % 2 == 0 ) {
                    ...
                    tmp_left += parseInt( $('div.fc-view-agendaWeek td.fc-col' + tmp_num).css('width') );
                    var odd_left = tmp_left - 9;
            ...
            $('div.fc-view-agendaDay').find('div.fc-day-separate').css('visibility', 'hidden');
        }
        ...
        function renderSlotSegs(segs, modifiedEventId) {
            ...
            slotScroller.mCustomScrollbar({
                ...
                //         for (i=0; i<colCnt; i++) {
                //          var currentCol = colBoundaries[i];
                //          if (currentCol.min!=null && 
                //              currentCol.min<=currentPosition+jumperReserve) {
                ...
                //          if (currentCol.max!=null && 
                //              currentCol.max>=currentPosition+slotScroller.height()-jumperReserve) {
            ...
            slotJumpersTop.each(function (i, jumper){
                $(jumper).unbind('click').click(function(){
                    ...
                    slotScroller.mCustomScrollbar("scrollTo", targetTop-t.getSlotHeight());
                });
            });
                ...
                $(jumper).unbind('click').click(function(){
                    ...
                    var tmp_scrollBot = 
                        targetPosition.bottom-targetPosition.top+t.getSlotHeight()>slotScroller.height()?
                            ...
                            targetPosition.bottom-slotScroller.height()+t.getSlotHeight()+1 
                                // +1 is a magic independent constant, 
                                //  used just to make the default scroll position look better

                    // slotScroller.scrollTop(tmp_scrollBot);
                    slotScroller.mCustomScrollbar("scrollTo", tmp_scrollBot);
                });
            });
        ...
        function draggableDayEvent (event, eventElement, isStart) {
            ...
            // console.log("// when event starts out FULL-DAY");
            eventElement.draggable ({
                zIndex: 9,
                scroll: false,
        ...
        function draggableSlotEvent(event, eventElement, timeElement) {
            ...
            eventElement.draggable({
                ...
                start: function (ev, ui) {
                    // console.log("timesolot drag start");
                    trigger('eventDragStart', eventElement, event, ev, ui);
                    //hideEvents(event, eventElement);
                ...
                drag: function (ev, ui) {
                    // console.log("timesolot drag ing");
                ...
                stop: function (ev, ui) {
                    // console.log("timesolot drag stop");
                    var cell = hoverListener.stop();
                    clearOverlays();
                    trigger('eventDragStop', eventElement, event, ev, ui);
    ...
    function countForwardSegs (levels) {
        // console.log("③fullcalendar_func countForwardSegs");
    ...
    function View (element, calendar, viewName) {
        // console.log("③fullcalendar_func View");
        var t = this;
        ...
        function eventElementHandlers(event, eventElement) {
            ...
            // console.log("eventElementHandlers's this is ", this);
            eventElement
                .click(function (ev){
                    // console.log("into click func");
                    // selectEvent(eventElement, true);
                    // console.log("click's this is ", this.id);\
                    selectEvent (eventElement, true);
                ...
                .hover(
                    function (ev) {
                        ...
                        // console.log("hover's this is ", this);
                ...
                //--@steve click=>dblclick--
                .dblclick(function (ev) {
                    if (!eventElement.hasClass('ui-draggable-dragging') &&
                        !eventElement.hasClass('ui-resizable-resizing')) {
                        
                        selectEvent (eventElement, true);
                        return trigger('eventDBClick', this, event, ev);
                    }
                });
    ...
    function DayEventRenderer () {
        // console.log("③fullcalendar_func DayEventRenderer");
        var t = this;
        ...
        function renderDaySegs(segs, modifiedEventId, isAllDay) {
            ...
            for (rowI=0; rowI<rowCnt; rowI++) {
                ...
                // console.log("arrayMax(colHeights): ",arrayMax(colHeights));
            }
        ...
        function daySegHTML(segs) { 
            ...
            for ( i=0; i<segCnt; i++ ) {
                ...
                left += 7
                titleWidth = right - left - 7 - 2;
                    // fc-event-inner fc-event-skin的宽度
                classes = classes.concat(event.className);
                if (event.source) {
                    classes = classes.concat(event.source.className || []);
                }
    ...
    function SelectionManager () {
        // console.log("③fullcalendar_func SelectionManager");
        var t = this;
    ...
    function OverlayManager () {
        // console.log("③fullcalendar_func OverlayManager");
        var t = this;
    ...
    function CoordinateGrid (buildFunc) {
        // console.log("③fullcalendar_func CoordinateGrid");
        var t = this;
        var rows;
        var cols;
    ...
    function HoverListener (coordinateGrid) {
        // console.log("③fullcalendar_func HoverListener");
        var t = this;
        var bindType;
        var change;
        var firstCell;
    ...
    function HorizontalPositionCache (getElement) {
        // console.log("③fullcalendar_func HorizontalPositionCache");
    ...
    function removeTodayClass (cell) {
        // console.log("③fullcalendar_func removeTodayClass");
        var classes = cell.attr('class').split(' ');
        var filter = ['fc-state-highlight', 'fc-today', 'fc-widget-content', 'fc-source-bg'];
        classes = $.grep(classes, function (el) {
            if ($.inArray(el, filter) > -1) {
    ...
    function getWeekNumber (date) {
        // console.log("③fullcalendar_func getWeekNumber");
    ...
    function ListEventRenderer () {
        // console.log("③fullcalendar_func ListEventRenderer");
        var t = this;
        ...
        function renderEventTime(event, seg) {
            ...
            if (segmode == 'smart') {
                if (event.start < seg.start) {
                    ...
                    datestr = formatDates(event.start, eventEnd, dateFormat + '{ â€?' + dateFormat + '}');
                ...
                datestr = formatDates(event.start, eventEnd, dateFormat + (duration > DAY_MS ? '{ â€?' + dateFormat + '}' : ''));
            }
    ...
    function ListView (element, calendar) {
        // console.log("③fullcalendar_func ListView");
        var t = this;
    ...
    function TableEventRenderer () {
        // console.log("③fullcalendar_func TableEventRenderer");
        var t = this;
        ...
        function scrollToDate (date) {
            ...
            if (currentDate.getDate() == 1) {
                // console.log("getListContainer().parent(): ",getListContainer().parent());
                console.log("11111111111");
                // getListContainer().parent().scrollTop(0);
                $('div.fc-list-content').mCustomScrollbar("scrollTo", "top");
            }
            else {
                // console.log("into eleseeeeeeeeeeeeeee");
                while (!segFound) {
                    ...
                    getListContainer().find('td.fc-list-header.fc-widget-header').each(function () {
                        ...
                        if (currSegHash == segHash) {
                            ...
                            // var top = $('div.fc-list-content').parent().scrollTop();
                            // console.log("top + offset: ", top + offset);
                            // getListContainer().parent().parent().scrollTop(top + offset);
                            $('div.fc-list-content').mCustomScrollbar("scrollTo", top + offset);
                        }
                    });

                    if (!segFound) {
                        // console.log("into if (!segFound)");
                        ...
                        if (nextDate.getDate() > currentDate.getDate()) {
                            ...
                            segFound = true;
                            // console.log("into else...segFound");
                            // getListContainer().parent().scrollTop(getListContainer().height());
                            $('div.fc-list-content').mCustomScrollbar("scrollTo", getListContainer().height());
                        }
                    }
                }
            }
        ...
        function renderEvents (events, modifiedEventId) {
            // console.log("function renderEvents");
        ...
        /** 
         * @desc 将对象 segs 中加入当月没有事件的天，用于显示
         * @author BuptStEve
         * @param {Object} segs
         * @return {Object} new_segs
         */
        function addNoneEvent(segs) {
            var new_segs = {};

            for (var i = 0; i < t.visEnd.getDate(); i++) {
                var colFormat = opt('columnFormat', t.name);
                var firstDay = opt('firstDay');
                var today = clearTime(new Date());
                var weekstart = addDays(cloneDate(today), - ((today.getDay() - firstDay + 7) % 7));
                var segDate = cloneDate(t.start);
                segDate.setDate(segDate.getDate() + i);
                var segHash = formatDate(segDate, colFormat);

                var dd = dayDiff(segDate, today);
                var wd = Math.floor(dayDiff(segDate, weekstart) / 7);
                var md = segDate.getMonth() + ((segDate.getYear() - today.getYear()) * 12) - today.getMonth();
                ...
                if (!(segHash in new_segs)) {
                    if (tmpObjKey === "") {
                        new_segs[segHash] = { events: [], start: segDate, title: segHash, daydiff: dd, weekdiff: wd, monthdiff: md };
                    }
                    else {
                        if (segDate < tmpObj.start) {
                            new_segs[segHash] = { events: [], start: segDate, title: segHash, daydiff: dd, weekdiff: wd, monthdiff: md };
                        } 
                        else {
                            new_segs[tmpObjKey] = tmpObj;
                            delete segs[tmpObjKey];
                        }
                    }
                }
            }
        ...
        function renderSegs (segs, modifiedEventId) {
            ...
            prevMonth = $('<tbody class="fc-list-header"><tr><td class="fc-list-header fc-month-nav fc-month-prev ' + headerClass + 
                        '" colspan="' + tableCols.length + '">' + opt('buttonText', 'prevMonth') + '</td></tr></tbody>')
                // .appendTo(table)
                ;
            ...
            for (j in segs) {
                ...
                if (seg.title) {
                    var segHeader = $('<tbody class="fc-list-header"><tr><td class="fc-list-header ' + headerClass + 
                                    '" colspan="' + tableCols.length + '">' + htmlEscape(seg.title) + '</td></tr></tbody>').appendTo(table);
                }
                segContainer = $('<tbody>').addClass('fc-list-section ' + contentClass).appendTo(table);
                s = '';
                segContainer.css({'width': parseInt(table.css('width')) - 30 - 21,
                                  'margin-left': 21});
                // console.log("table.css('width');: ", table.css('width'));
                ...
                if (seg.events.length === 0) {
                   s = "<tbody class='fc-list-section fc-widget-content fc-day-1' style='width: 762px; margin-left: 21px;'> \
                            <tr class='fc-mon fc-event fc-event-row event_item fc-first fc-last' data-res-id='' data-id=''> \
                                <td class='fc-event-handle' style='background-color:#999;border-color:#b3d1eb;color:#404040'></td> \
                                <td class='fc-event-date' colspan='1'></td> \
                                <td class='fc-event-time' style='text-overflow: ellipsis; overflow: hidden; color: #999'>本日无事件</td> \
                                <td class='fc-event-title'></td> \
                            </tr> \
                        </tbody>";
            ...
            nextMonth = $('<tbody class="fc-list-header"><tr><td class="fc-list-header fc-month-nav fc-month-next ' + headerClass + 
                        '" colspan="' + tableCols.length + '">' + opt('buttonText', 'nextMonth') + '</td></tr></tbody>')
                // .appendTo(table)
                ;
    ...
    function TableView (element, calendar) {
        // console.log("③fullcalendar_func TableView");
        var t = this;
        ...
        function buildSkeleton (date) {
            ...
            $('div.fc-list-content div.mCustomScrollBox, div.fc-list-content div.mCustomScrollBox div.mCSB_container').css('overflow', 'visible');
        }
        ...
        function setHeight(height, dateChanged) {
            ...
            div.css({'height': (height-div.position().top-2)+'px',
                     'overflow': 'visible'});
        }

        function setWidth (width) {
            var outerWidth = Math.floor(element.parent().width() / 2) - 21;
            element.css({
                'width': outerWidth,
                'padding-top': 20,
                'padding-left': 21
